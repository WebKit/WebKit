...
unsigned int if_calloc_counter = 0;
unsigned int if_realloc_counter = 0;

size_t space_wastage = 0;
...
void *(*oldCalloc)(struct _malloc_zone_t *zone, size_t num_items, size_t size);
void *(*oldRealloc)(struct _malloc_zone_t *zone, void *ptr, size_t size);
void (*oldFree)(struct _malloc_zone_t *zone, void *ptr);
...
size_t if_size(struct _malloc_zone_t *zone, const void *ptr);


void *if_malloc (struct _malloc_zone_t *zone, size_t size);


void *if_calloc(struct _malloc_zone_t *zone, size_t num_items, size_t size);
...
{
    fprintf (stdout, "if_malloc_counter = %d\n", if_malloc_counter);
    fprintf (stdout, "if_calloc_counter = %d\n", if_calloc_counter);
    fprintf (stdout, "if_realloc_counter = %d\n", if_realloc_counter);
    fprintf (stdout, "total_space = %ld\n", total_space);
    fprintf (stdout, "space_wastage = %ld\n", space_wastage);
}


const void *if_cf_retain(const void *info);
const void *if_cf_retain(const void *info)
{
    return info;
}

void if_cf_release(const void *info);
void if_cf_release(const void *info)
{
    //return CFRelease(info);
}

void *if_cf_alloc(CFIndex size, CFOptionFlags hint, void *info);
void *if_cf_alloc(CFIndex size, CFOptionFlags hint, void *info)
{
    void *memory;

    memory = if_malloc (if_zone, size);

    return memory;
}

void *if_cf_realloc(void *ptr, CFIndex newsize, CFOptionFlags hint, void *info);
void *if_cf_realloc(void *ptr, CFIndex newsize, CFOptionFlags hint, void *info)
{
    void *memory;

    memory = if_realloc (if_zone, ptr, newsize);

    return memory;
}

void if_cf_dealloc(void *ptr, void *info);
void if_cf_dealloc(void *ptr, void *info)
{
    if_free (if_zone, ptr);
}

CFIndex if_cf_preferredSize(CFIndex size, CFOptionFlags hint, void *info);
CFIndex if_cf_preferredSize(CFIndex size, CFOptionFlags hint, void *info)
{
    return size;
}
...
void setupDebugMalloc(void)
{
    ...
    if (if_zone->malloc != if_malloc){
        ...
        oldRealloc = if_zone->realloc;
        oldFree = if_zone->free;
        oldSize = if_zone->size;
        if_zone->malloc = if_malloc;
        if_zone->calloc = if_calloc;
        if_zone->valloc = if_valloc;
        if_zone->realloc = if_realloc;
        if_zone->free = if_free;
        if_zone->size = if_size;

#ifdef REPLACE_CF_ALLOCATOR
        if (!allocator) {
            CFAllocatorContext context = {0, NULL, if_cf_retain, if_cf_release, NULL, if_cf_alloc, if_cf_realloc, if_cf_dealloc, if_cf_preferredSize};
...
{
    malloc_zone_t *if_zone;

    if_zone = malloc_default_zone();
    if (if_zone->malloc == if_malloc){
        if_zone->malloc = oldMalloc;
        if_zone->calloc = oldCalloc;
        if_zone->realloc = oldRealloc;
        if_zone->free = if_free;
    }
}
...
#define MAX_POOL_ENTRIES 1024*4
#define MINIMUM_MORECORE_SIZE  (8 * 1024 * 1024)
static int next_os_pool = 0;
void *our_os_pools[MAX_POOL_ENTRIES];
int our_os_pool_sizes[MAX_POOL_ENTRIES];
...
void *osMoreCore(int size)
{
    ...
    if (size > 0)
    {
        ...
        err = vm_allocate(mach_task_self(), &addr, (vm_size_t)size, VM_MAKE_TAG(VM_MEMORY_MALLOC_HUGE) | 1);
        ptr = (void *)addr;
        if (ptr == 0)
        {
        ...
        our_os_pool_sizes[next_os_pool] = size;
        next_os_pool++;
...
int isMine(void *ptr)
{
    ...
    for (i = 0; i < next_os_pool; i++){
        if (ptr >= our_os_pools[i] && ptr < (void *)((char *)our_os_pools[i] + our_os_pool_sizes[i]))
            return 1;
    }
    return 0;
}
