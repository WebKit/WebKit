// Smoke test for the RTT cycle-anchoring (m_referencedRTTs) design introduced
// on the rtt-uniform branch. Each RTT holds Ref<const RTT> for every other
// RTT it references via field/element/param/return types, so intra-recgroup
// mutual references form a refcount cycle that is intentionally leaked
// rather than collected (no UAF, but no reclaim either).
//
...
// later uses. Now they keep each other alive via m_referencedRTTs.
//
// See: Source/JavaScriptCore/wasm/WasmTypeDefinition.h:RTTFunctionPayload /
// RTTStructPayload / RTTArrayPayload m_referencedRTTs.
