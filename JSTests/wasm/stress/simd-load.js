...
(module
    ...
    (func (export "load_and_store_lane") (result i64)
        (v128.store16_lane 1 (i32.const 0) ;; In memory as 85 FF 82...
            (v128.load8_lane 2 (i32.const 5) ;; FF FF 85 FF FF.... or FFFF FF85 FFFF....
...
async function test() {
    ...
    for (let i = 0; i < 10000; ++i) {
        assert.eq(load_simple(), 0xAB)
        assert.eq(store_simple(), 0xEFBDn)
        deBruijin()
        assert.eq(load_and_store_lane() + (1n<<64n), 0x878685848382FF85n)
    }
}
