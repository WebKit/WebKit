...
for (const op of WASM.opcodes("memory")) {
    ...
    for (let alignLog2 = 0; alignLog2 < 16; ++alignLog2) {
        ...
        if (alignLog2 <= maxAlignLog2)
            ...
            assert.compileError(builder.WebAssembly().get(), `WebAssembly.Module doesn't parse at byte ${start}: byte alignment ${1 << alignLog2} exceeds ${info.type}'s natural alignment ${1 << maxAlignLog2}, in function at index 0`);
