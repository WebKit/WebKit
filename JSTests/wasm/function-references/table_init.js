...
async function testTableInitParsing() {
   ...
   */
  assert.compileError(assert.stringToBytes("\x00\x61\x73\x6d\x01\x00\x00\x00\x04\x89\x80\x80\x80\x00\x01\x40\x00\x6f\x00\x0a\xd0\x70\x0b"),
   ...
   */
  assert.compileError(assert.stringToBytes("\x00\x61\x73\x6d\x01\x00\x00\x00\x02\x89\x80\x80\x80\x00\x01\x01\x6d\x01\x79\x01\x40\x00\x6f\x00\x0a\xd0\x70\x0b"),
    "WebAssembly.Module doesn't parse at byte 21: can't parse Table type")

  // Marker byte for initialized table must be 0x40
  assert.compileError(assert.stringToBytes("\x00\x61\x73\x6d\x01\x00\x00\x00\x04\x89\x80\x80\x80\x00\x01\x41\x00\x6f\x00\x0a\xd0\x70\x0b"),
    "WebAssembly.Module doesn't parse at byte 16: can't parse Table type")

  // Reserved byte should be zero
  assert.compileError(assert.stringToBytes("\x00\x61\x73\x6d\x01\x00\x00\x00\x04\x89\x80\x80\x80\x00\x01\x40\x01\x6f\x00\x0a\xd0\x70\x0b"),
    "WebAssembly.Module doesn't parse at byte 17: can't parse explicitly initialized Table's reserved byte")

  // Missing table type
  assert.compileError(assert.stringToBytes("\x00\x61\x73\x6d\x01\x00\x00\x00\x04\x83\x80\x80\x80\x00\x01\x40\x00"),
    "WebAssembly.Module doesn't parse at byte 17: can't parse Table type")

  // Truncated table type
  assert.compileError(assert.stringToBytes("\x00\x61\x73\x6d\x01\x00\x00\x00\x04\x84\x80\x80\x80\x00\x01\x40\x00\x6f"),
    "WebAssembly.Module doesn't parse at byte 18: can't parse resizable limits flags")

  // Missing init expr
  assert.compileError(assert.stringToBytes("\x00\x61\x73\x6d\x01\x00\x00\x00\x04\x86\x80\x80\x80\x00\x01\x40\x00\x6f\x00\x0a"),
    "WebAssembly.Module doesn't parse at byte 20: can't get init_expr's opcode")

  // Init expr immediately ends with end marker
  assert.compileError(assert.stringToBytes("\x00\x61\x73\x6d\x01\x00\x00\x00\x04\x87\x80\x80\x80\x00\x01\x40\x00\x6f\x00\x0a\x0b"),
