...
from framework.parallel_runner import ParallelWebAssemblyDebuggerTestRunner
from framework.auto_discovery import create_auto_registered_runner, TestCaseDiscovery
...
def create_test_runner(verbose=False, build_config=None, parallel=False, workers=None, test_names=None):
    ...
    if parallel:
        ...
        temp_runner = create_auto_registered_runner(build_config=build_config, verbose_wasm_debugger=False)
        
        # Calculate test count
        if test_names:
            test_count = len([name for name in test_names if temp_runner.registry.get_test_class(name)])
        else:
        ...
        if verbose:
            Logger.info(f"Smart parallel execution: {test_count} tests, {smart_workers} workers (CPU cores: {os.cpu_count()})")
        ...
        runner = create_auto_registered_runner(build_config=build_config, verbose_wasm_debugger=False)
    return runner
...
def main():
    ...
    parser.add_argument("--pid-tid", action="store_true", help="Enable PID/TID prefixes")
    parser.add_argument("--test", "-t", action="append", help="Run specific test case(s)")
    parser.add_argument("--list", "-l", action="store_true", help="List available test cases")
    ...
    runner = create_test_runner(args.verbose, build_config, parallel, workers, args.test)
    ...
    try:
        if args.test:
            results = runner.run_specific_tests_parallel(args.test) if parallel else runner.run_specific_tests(args.test)
        else:
            results = runner.run_all_tests_parallel() if parallel else runner.run_all_tests()
