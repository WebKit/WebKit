...
class ParallelWebAssemblyDebuggerTestRunner:
    ...
    def __init__(self, build_config: Optional[str] = None, verbose_wasm_debugger: bool = False, max_workers: int = 4, verbose_parallel: bool = False):
        self.logger = Logger()
    ...
    def run_all_tests_parallel(self) -> List[TestResult]:
        ...
        if not test_names:
            ...
            return []
        return self._run_tests_parallel(test_names, "Starting Parallel WebAssembly Debugger Test Suite")
    ...
    def run_specific_tests_parallel(self, test_names: List[str]) -> List[TestResult]:
        ...
        # Validate test names
        valid_test_names = [name for name in test_names if self.registry.get_test_class(name)]
    ...
    def _run_tests_parallel(self, test_names: List[str], header_msg: str) -> List[TestResult]:
        ...
        for test_name in test_names:
            ...
            self.logger.verbose(f"📋 Pre-allocated port {port_assignments[test_name]} for test {test_name}")
        
        # Run tests in parallel
        with ThreadPoolExecutor(max_workers=min(self.max_workers, len(test_names))) as executor:
            future_to_test = {
                executor.submit(self._execute_single_test, name, port_assignments[name]): (name, port_assignments[name])
            ...
            for future in as_completed(future_to_test):
                ...
                try:
                    ...
                    self.logger.info(f"{status} {test_name} (Port: {port}, Time: {result.duration():.2f}s)")
    ...
    def _execute_single_test(self, test_name: str, port: int) -> TestResult:
        ...
        try:
            ...
            if hasattr(test_instance, 'set_process_tracker'):
                test_instance.set_process_tracker(self._process_thread_tracker, worker_tid)
            ...
            status = "passed" if result.success else "failed"
            self.logger.parallel_step(f"Test {test_name} {status} ({result.duration():.2f}s)")
    ...
    def cleanup(self):
        ...
        try:
            subprocess.run("pkill -f 'jsc.*--wasm-debug'", shell=True, capture_output=True, timeout=5)
...
class ProcessThreadTracker:
    ...
    def __init__(self):
        self.main_pid = None
        self.main_tid = None
        self.worker_threads = {}  # tid -> {test_cases: [], current_test: str, processes: {}, monitor_threads: []}
    ...
    def track_monitor_thread(self, worker_tid: int, monitor_tid: int, process_type: str, stream_type: str):
    ...
    def print_summary(self):
        ...
        for worker_tid, data in self.worker_threads.items():
            ...
            logger.info(f"  PID: {self.main_pid} TID: {worker_tid} (worker thread {worker_count})")
            ...
            for key, pid in processes.items():
                if '_' in key:
                    ...
                    if test_case not in test_case_resources:
                        test_case_resources[test_case] = {'processes': {}, 'monitor_threads': []}
                ...
                if test_case in test_case_resources:
                    ...
                    if resources['monitor_threads']:
                        ...
                        for monitor in resources['monitor_threads']:
                            logger.info(f"        PID: {self.main_pid} TID: {monitor['tid']} ({monitor['process_type']} {monitor['stream_type']})")
        ...
        logger.info(f"  Framework Threads: {1 + worker_count} (1 main + {worker_count} workers)")
