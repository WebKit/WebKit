...
class TestCaseDiscovery:
    ...
    def __init__(self, test_cases_dir: str = None):
        ...
        if test_cases_dir is None:
            # Default to the test_cases directory relative to this file
            current_dir = Path(__file__).parent
            self.test_cases_dir = current_dir / "test_cases"
        else:
    ...
    def discover_test_cases(self) -> Dict[str, Type[BaseTestCase]]:
        ...
        if not self.test_cases_dir.exists():
            self.logger.warning(f"Test cases directory not found: {self.test_cases_dir}")
        ...
        for py_file in python_files:
            ...
            try:
                # Import the module
                module_name = f"framework.test_cases.{py_file.stem}"
    ...
    def _extract_test_classes(self, module) -> Dict[str, Type[BaseTestCase]]:
        ...
        for name, obj in inspect.getmembers(module):
            if (inspect.isclass(obj) and 
                ...
                obj != BaseTestCase and  # Don't include the base class itself
...
def create_auto_registered_runner(build_config: str = None, verbose_wasm_debugger: bool = False):
    ...
    runner = WebAssemblyDebuggerTestRunner(build_config=build_config, verbose_wasm_debugger=verbose_wasm_debugger)
