...
class BaseTestCase:
    ...
    def run(self) -> TestResult:
        ...
        try:
            ...
            self.logger.error(f"Test {self.name} failed ({result.duration():.2f}s): {e}")
        finally:
            self.teardown()
        return result
    ...
    def start_debugger(self, test_file: str) -> bool:
        ...
        try:
            ...
            if self._process_tracker and self._worker_tid:
                self._process_tracker.track_process(self._worker_tid, "JSC", self.debugger_process.pid)
    ...
    def start_lldb(self, connection_timeout: float = 10.0) -> bool:
        ...
        try:
            ...
            # Track LLDB process
            if self._process_tracker and self._worker_tid:
                self._process_tracker.track_process(self._worker_tid, "LLDB", self.lldb_process.pid)
    ...
    def wait_for_lldb_stop(self, timeout: float = 10.0) -> bool:
        ...
        result = self.wait_for_lldb_patterns(["Process 1 stopped"], PatternMatchMode.ANY, timeout)
    ...
    def wait_for_lldb_patterns(
        ...
        if pattern_matched and result.success:
            ...
            self.logger.verbose(f"🎯 LLDB patterns matched ({mode.value} mode): {', '.join(matched_pattern_details)}")
        else:
            result.success = False
            result.timeout = True
            result.completion_reason = f"Timeout after {timeout}s"
            self.logger.warning(f"⏰ Timeout waiting for LLDB patterns after {timeout}s")
                ...
                for line in iter(stream.readline, ''):
                    ...
                    if pattern_matching:
                        ...
                        try:
                            ...
                            self.logger.warning(f"LLDB queue full, may miss pattern: {line}")
                    
                    from .utils import Colors, Logger as UtilsLogger
                    if UtilsLogger._verbose:
                        print(f"{Colors.DIM}🔍 [{process_name}][{stream_name.upper()}] {line}{Colors.RESET}")
    ...
    def _start_debugger_monitoring(self):
        ...
        self._start_process_monitoring(self.debugger_process, "Debugger", pattern_matching=False)
