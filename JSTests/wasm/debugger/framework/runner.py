...
class WebAssemblyDebuggerTestRunner:
    ...
    def __init__(self, build_config: Optional[str] = None, verbose_wasm_debugger: bool = False):
        self.logger = Logger()
    ...
    def _run_single_test(self, test_name: str) -> TestResult:
        ...
        try:
            ...
            test_instance = self.registry.create_test_instance(test_name, self.build_config, port=port)
    ...
    def _print_summary(self):
        ...
        script_runtime = time.time() - self.script_start_time if self.script_start_time else 0
    ...
    def cleanup(self):
        ...
        import subprocess
        try:
            subprocess.run("pkill -f 'jsc.*--wasm-debug'", shell=True, capture_output=True)
        except Exception:
            pass
