// Copyright (C) 2025 Igalia, S.L. All rights reserved.
// This code is governed by the BSD license found in the LICENSE file.

/*---
esid: sec-temporal.plaindatetime.prototype.subtract
description: >
  Check various basic calculations not involving leap years or constraining
  (roc calendar)
features: [Temporal, Intl.Era-monthcode]
includes: [temporalHelpers.js]
---*/

const calendar = "roc";

const years1 = new Temporal.Duration(-1);
const years1n = new Temporal.Duration(1);
const years4 = new Temporal.Duration(-4);
const years4n = new Temporal.Duration(4);

const date1110716 = Temporal.PlainDateTime.from({ year: 111, monthCode: "M07", day: 16, hour: 12, minute: 34, calendar });

TemporalHelpers.assertPlainDateTime(
  date1110716.subtract(years1),
  112, 7, "M07", 16, 12, 34, 0, 0, 0, 0, "add 1y",
  "roc", 112);
TemporalHelpers.assertPlainDateTime(
  date1110716.subtract(years4),
  115, 7, "M07", 16, 12, 34, 0, 0, 0, 0, "add 4y",
  "roc", 115);

TemporalHelpers.assertPlainDateTime(
  date1110716.subtract(years1n),
  110, 7, "M07", 16, 12, 34, 0, 0, 0, 0, "subtract 1y",
  "roc", 110);
TemporalHelpers.assertPlainDateTime(
  date1110716.subtract(years4n),
  107, 7, "M07", 16, 12, 34, 0, 0, 0, 0, "subtract 4y",
  "roc", 107);


// Months

const months5 = new Temporal.Duration(0, -5);
const months5n = new Temporal.Duration(0, 5);
const years1months2 = new Temporal.Duration(-1, -2);
const years1months2n = new Temporal.Duration(1, 2);

TemporalHelpers.assertPlainDateTime(
  date1110716.subtract(months5),
  111, 12, "M12", 16, 12, 34, 0, 0, 0, 0, "add 5mo with result in the same year",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M08", day: 16, hour: 12, minute: 34, calendar }).subtract(months5),
  112, 1, "M01", 16, 12, 34, 0, 0, 0, 0, "add 5mo with result in the next year",
  "roc", 112);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 109, monthCode: "M10", day: 1, hour: 12, minute: 34, calendar }).subtract(months5),
  110, 3, "M03", 1, 12, 34, 0, 0, 0, 0, "add 5mo with result in the next year on day 1 of month",
  "roc", 110);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M10", day: 31, hour: 12, minute: 34, calendar }).subtract(months5),
  112, 3, "M03", 31, 12, 34, 0, 0, 0, 0, "add 5mo with result in the next year on day 31 of month",
  "roc", 112);

TemporalHelpers.assertPlainDateTime(
  date1110716.subtract(years1months2),
  112, 9, "M09", 16, 12, 34, 0, 0, 0, 0, "add 1y 2mo",
  "roc", 112);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M11", day: 30, hour: 12, minute: 34, calendar }).subtract(years1months2),
  113, 1, "M01", 30, 12, 34, 0, 0, 0, 0, "add 1y 2mo with result in the next year",
  "roc", 113);

TemporalHelpers.assertPlainDateTime(
  date1110716.subtract(months5n),
  111, 2, "M02", 16, 12, 34, 0, 0, 0, 0, "subtract 5mo with result in the same year",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M01", day: 16, hour: 12, minute: 34, calendar }).subtract(months5n),
  110, 8, "M08", 16, 12, 34, 0, 0, 0, 0, "subtract 5mo with result in the previous year",
  "roc", 110);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 109, monthCode: "M02", day: 1, hour: 12, minute: 34, calendar }).subtract(months5n),
  108, 9, "M09", 1, 12, 34, 0, 0, 0, 0, "subtract 5mo with result in the previous year on day 1 of month",
  "roc", 108);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M03", day: 31, hour: 12, minute: 34, calendar }).subtract(months5n),
  110, 10, "M10", 31, 12, 34, 0, 0, 0, 0, "subtract 5mo with result in the previous year on day 31 of month",
  "roc", 110);

TemporalHelpers.assertPlainDateTime(
  date1110716.subtract(years1months2n),
  110, 5, "M05", 16, 12, 34, 0, 0, 0, 0, "subtract 1y 2mo",
  "roc", 110);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M02", day: 17, hour: 12, minute: 34, calendar }).subtract(years1months2n),
  109, 12, "M12", 17, 12, 34, 0, 0, 0, 0, "subtract 1y 2mo with result in the previous year",
  "roc", 109);

// Weeks

const weeks1 = new Temporal.Duration(0, 0, -1);
const weeks1n = new Temporal.Duration(0, 0, 1);
const weeks6 = new Temporal.Duration(0, 0, -6);
const weeks6n = new Temporal.Duration(0, 0, 6);
const years1weeks2 = new Temporal.Duration(-1, 0, -2);
const years1weeks2n = new Temporal.Duration(1, 0, 2);
const months2weeks3 = new Temporal.Duration(0, -2, -3);
const months2weeks3n = new Temporal.Duration(0, 2, 3);

const date1101228 = Temporal.PlainDateTime.from({ year: 110, monthCode: "M12", day: 28, hour: 12, minute: 34, calendar });
const date1110127 = Temporal.PlainDateTime.from({ year: 111, monthCode: "M01", day: 27, hour: 12, minute: 34, calendar });
const date1110219 = Temporal.PlainDateTime.from({ year: 111, monthCode: "M02", day: 19, hour: 12, minute: 34, calendar });
const date1110604 = Temporal.PlainDateTime.from({ year: 111, monthCode: "M06", day: 4, hour: 12, minute: 34, calendar });
const date1110627 = Temporal.PlainDateTime.from({ year: 111, monthCode: "M06", day: 27, hour: 12, minute: 34, calendar });
const date1110727 = Temporal.PlainDateTime.from({ year: 111, monthCode: "M07", day: 27, hour: 12, minute: 34, calendar });
const date1111224 = Temporal.PlainDateTime.from({ year: 111, monthCode: "M12", day: 24, hour: 12, minute: 34, calendar });

TemporalHelpers.assertPlainDateTime(
  date1110219.subtract(weeks1),
  111, 2, "M02", 26, 12, 34, 0, 0, 0, 0, "add 1w",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  date1111224.subtract(weeks1),
  111, 12, "M12", 31, 12, 34, 0, 0, 0, 0, "add 1w with result on the last day of the year",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M12", day: 25, hour: 12, minute: 34, calendar }).subtract(weeks1),
  112, 1, "M01", 1, 12, 34, 0, 0, 0, 0, "add 1w with result on the first day of the next year",
  "roc", 112);

TemporalHelpers.assertPlainDateTime(
  date1110127.subtract(weeks1),
  111, 2, "M02", 3, 12, 34, 0, 0, 0, 0, "add 1w in a 31-day month with result in the next month",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  date1110727.subtract(weeks1),
  111, 8, "M08", 3, 12, 34, 0, 0, 0, 0, "add 1w in another 31-day month with result in the next month",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  date1110627.subtract(weeks1),
  111, 7, "M07", 4, 12, 34, 0, 0, 0, 0, "add 1w in a 30-day month with result in the next month",
  "roc", 111);

TemporalHelpers.assertPlainDateTime(
  date1110127.subtract(weeks6),
  111, 3, "M03", 10, 12, 34, 0, 0, 0, 0, "add 6w with result in the same year",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  date1111224.subtract(weeks6),
  112, 2, "M02", 4, 12, 34, 0, 0, 0, 0, "add 6w with result in the next year",
  "roc", 112);
TemporalHelpers.assertPlainDateTime(
  date1110627.subtract(weeks6),
  111, 8, "M08", 8, 12, 34, 0, 0, 0, 0, "add 6w crossing months of 30 and 31 days",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  date1110727.subtract(weeks6),
  111, 9, "M09", 7, 12, 34, 0, 0, 0, 0, "add 6w crossing months of 31 and 31 days",
  "roc", 111);

TemporalHelpers.assertPlainDateTime(
  date1101228.subtract(years1weeks2),
  112, 1, "M01", 11, 12, 34, 0, 0, 0, 0, "add 1y 2w with result in the next year",
  "roc", 112);

TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 109, monthCode: "M10", day: 28, hour: 12, minute: 34, calendar }).subtract(months2weeks3),
  110, 1, "M01", 18, 12, 34, 0, 0, 0, 0, "add 2mo 3w with result in the next year",
  "roc", 110);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 109, monthCode: "M10", day: 31, hour: 12, minute: 34, calendar }).subtract(months2weeks3),
  110, 1, "M01", 21, 12, 34, 0, 0, 0, 0, "add 2mo 3w with result in the next year",
  "roc", 110);

TemporalHelpers.assertPlainDateTime(
  date1110219.subtract(weeks1n),
  111, 2, "M02", 12, 12, 34, 0, 0, 0, 0, "subtract 1w",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M01", day: 8, hour: 12, minute: 34, calendar }).subtract(weeks1n),
  111, 1, "M01", 1, 12, 34, 0, 0, 0, 0, "subtract 1w with result on the first day of the year",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M01", day: 7, hour: 12, minute: 34, calendar }).subtract(weeks1n),
  110, 12, "M12", 31, 12, 34, 0, 0, 0, 0, "subtract 1w with result on the last day of the previous year",
  "roc", 110);

TemporalHelpers.assertPlainDateTime(
  date1110604.subtract(weeks1n),
  111, 5, "M05", 28, 12, 34, 0, 0, 0, 0, "subtract 1w with result in the previous 31-day month",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M07", day: 3, hour: 12, minute: 34, calendar }).subtract(weeks1n),
  111, 6, "M06", 26, 12, 34, 0, 0, 0, 0, "subtract 1w with result in the previous 30-day month",
  "roc", 111);

TemporalHelpers.assertPlainDateTime(
  date1110604.subtract(weeks6n),
  111, 4, "M04", 23, 12, 34, 0, 0, 0, 0, "subtract 6w with result in the same year",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  date1110127.subtract(weeks6n),
  110, 12, "M12", 16, 12, 34, 0, 0, 0, 0, "subtract 6w with result in the previous year",
  "roc", 110);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M09", day: 8, hour: 12, minute: 34, calendar }).subtract(weeks6n),
  111, 7, "M07", 28, 12, 34, 0, 0, 0, 0, "subtract 6w crossing months of 30 and 31 days",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M08", day: 8, hour: 12, minute: 34, calendar }).subtract(weeks6n),
  111, 6, "M06", 27, 12, 34, 0, 0, 0, 0, "subtract 6w crossing months of 31 and 31 days",
  "roc", 111);

TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 112, monthCode: "M01", day: 5, hour: 12, minute: 34, calendar }).subtract(years1weeks2n),
  110, 12, "M12", 22, 12, 34, 0, 0, 0, 0, "subtract 1y 2w with result in the previous year",
  "roc", 110);

TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 109, monthCode: "M03", day: 2, hour: 12, minute: 34, calendar }).subtract(months2weeks3n),
  108, 12, "M12", 12, 12, 34, 0, 0, 0, 0, "subtract 2mo 3w with result in the previous year",
  "roc", 108);

// Days

const days10 = new Temporal.Duration(0, 0, 0, -10);
const days10n = new Temporal.Duration(0, 0, 0, 10);
const weeks2days3 = new Temporal.Duration(0, 0, -2, -3);
const weeks2days3n = new Temporal.Duration(0, 0, 2, 3);
const years1months2days4 = new Temporal.Duration(-1, -2, 0, -4);
const years1months2days4n = new Temporal.Duration(1, 2, 0, 4);

TemporalHelpers.assertPlainDateTime(
  date1110716.subtract(days10),
  111, 7, "M07", 26, 12, 34, 0, 0, 0, 0, "add 10 days with result in the same month",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M07", day: 26, hour: 12, minute: 34, calendar }).subtract(days10),
  111, 8, "M08", 5, 12, 34, 0, 0, 0, 0, "add 10 days with result in the next month",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M12", day: 26, hour: 12, minute: 34, calendar }).subtract(days10),
  112, 1, "M01", 5, 12, 34, 0, 0, 0, 0, "add 10 days with result in the next year",
  "roc", 112);

TemporalHelpers.assertPlainDateTime(
  date1101228.subtract(weeks2days3),
  111, 1, "M01", 14, 12, 34, 0, 0, 0, 0, "add 2w 3d with result in the next year",
  "roc", 111);

TemporalHelpers.assertPlainDateTime(
  date1110716.subtract(years1months2days4),
  112, 9, "M09", 20, 12, 34, 0, 0, 0, 0, "add 1y 2mo 4d",
  "roc", 112);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M02", day: 27, hour: 12, minute: 34, calendar }).subtract(years1months2days4),
  112, 5, "M05", 1, 12, 34, 0, 0, 0, 0, "add 1y 2mo 4d with result in a month following a 30-day month",
  "roc", 112);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M07", day: 30, hour: 12, minute: 34, calendar }).subtract(years1months2days4),
  112, 10, "M10", 4, 12, 34, 0, 0, 0, 0, "add 1y 2mo 4d with result in a month following a 30-day month",
  "roc", 112);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M01", day: 28, hour: 12, minute: 34, calendar }).subtract(years1months2days4),
  112, 4, "M04", 1, 12, 34, 0, 0, 0, 0, "add 1y 2mo 4d with result in a month following a 31-day month",
  "roc", 112);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M06", day: 30, hour: 12, minute: 34, calendar }).subtract(years1months2days4),
  112, 9, "M09", 3, 12, 34, 0, 0, 0, 0, "add 1y 2mo 4d with result in a month following a 31-day month",
  "roc", 112);

TemporalHelpers.assertPlainDateTime(
  date1110716.subtract(days10n),
  111, 7, "M07", 6, 12, 34, 0, 0, 0, 0, "subtract 10 days with result in the same month",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M07", day: 6, hour: 12, minute: 34, calendar }).subtract(days10n),
  111, 6, "M06", 26, 12, 34, 0, 0, 0, 0, "subtract 10 days with result in the previous month",
  "roc", 111);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M01", day: 4, hour: 12, minute: 34, calendar }).subtract(days10n),
  110, 12, "M12", 25, 12, 34, 0, 0, 0, 0, "subtract 10 days with result in the previous year",
  "roc", 110);

TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M01", day: 15, hour: 12, minute: 34, calendar }).subtract(weeks2days3n),
  110, 12, "M12", 29, 12, 34, 0, 0, 0, 0, "subtract 2w 3d with result in the previous year",
  "roc", 110);

TemporalHelpers.assertPlainDateTime(
  date1110716.subtract(years1months2days4n),
  110, 5, "M05", 12, 12, 34, 0, 0, 0, 0, "subtract 1y 2mo 4d",
  "roc", 110);
TemporalHelpers.assertPlainDateTime(
  Temporal.PlainDateTime.from({ year: 111, monthCode: "M07", day: 4, hour: 12, minute: 34, calendar }).subtract(years1months2days4n),
  110, 4, "M04", 30, 12, 34, 0, 0, 0, 0, "subtract 1y 2mo 4d with result in a 30-day month",
  "roc", 110);
TemporalHelpers.assertPlainDateTime(
  date1110604.subtract(years1months2days4n),
  110, 3, "M03", 31, 12, 34, 0, 0, 0, 0, "subtract 1y 2mo 4d with result in a 31-day month",
  "roc", 110);
