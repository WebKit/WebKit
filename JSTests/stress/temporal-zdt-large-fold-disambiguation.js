...
// Regression tests for large-fold disambiguation in ZonedDateTime.
//
// Bug fixed: kMaxFoldWindowMs in getPossibleEpochNanosecondsFor was 7,200,000 ms (2 hours).
// For 24-hour date-line crossing folds (Alaska 1867 Purchase, Pacific/Apia 1892, etc.) the
// fold transition is up to ~24h from icuEpochMs, far beyond the 2h window.
// Result: tryFoldFromTransition returned false → only 1 candidate found → disambiguation
// always returned the post-fold (later) candidate, even when 'earlier' was requested.
// Fix: expanded kMaxFoldWindowMs to 90,000,000 ms (25 hours). The verification step
// (verifyOffset == otherOffset) prevents false positives from unrelated transitions.
...
// This was the primary failing case (earlier → wrong offset -11:47).
{
    ...
    // with({offset}) switching between the two occurrences
...
// Same Alaska Purchase event, different Alaskan city. -24h fall-back.
{
    ...
    // Earlier: pre-fold (east of date line, positive offset)
    // Later: post-fold (west of date line, negative offset)
...
// ─── Normal 1h DST fold still works (regression check) ─────────────────────
...
// ─── Transition invariants: offset at transition point is the NEW offset ────
{
...
// ─── Pacific/Apia 1892 (24h fall-back, Samoa date-line crossing) ─────────────
...
// ─── Antarctica/Davis 1957 (7h spring-forward, station first established) ───
...
// ─── Antarctica/Casey 1969 (8h spring-forward) ──────────────────────────────
{
