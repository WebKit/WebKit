...
const source = "async-stack-trace-promise-allKeyed-basic.js";
...

{
  ...
  async function run() { await Promise.allKeyed({ a: fine(), b: thrower() }); }
...

{
  ...
  async function runner() { await Promise.allKeyed({ t1: task1(), t2: task2(), t3: task3() }); }
...

{
  ...
  async function bar() {
    await 1;
    await Promise.allKeyed({ a: task1(), b: baz() });
  }
  async function baz() {
    await 2;
    throw new Error('task2 error');
