...
// --- Duration.round with PlainDate relativeTo ---
{
    ...
    // Jan has 31 days. 1 month + 15 days = 1.48 months → rounds to 2 (halfExpand rounds 0.48 to 1, so 1+1=2)
...
// --- Duration.total with PlainDate relativeTo ---
{
...
// --- Duration.round with ZDT relativeTo (DST-aware) ---
{
    const d = Temporal.Duration.from({ hours: 24 });
    const plainResult = d.round({
        largestUnit: "days",
        relativeTo: Temporal.PlainDate.from("2024-03-10"),
    });
    shouldBe(plainResult.days, 1, "24h = 1 day with PlainDate relativeTo");

    const zdtResult = d.round({
        largestUnit: "days",
        relativeTo: Temporal.ZonedDateTime.from("2024-03-10T00:00[America/New_York]"),
    });
    // Spring-forward day is only 23h, so 24h > 1 day in ZDT terms
...
// --- Duration.compare with ZDT relativeTo ---
{
    const d1 = Temporal.Duration.from({ days: 1 });
    const d2 = Temporal.Duration.from({ hours: 24 });

    const plainCmp = Temporal.Duration.compare(d1, d2, {
        relativeTo: Temporal.PlainDate.from("2024-03-10"),
    });
