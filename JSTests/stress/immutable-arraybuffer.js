...
{
    // A store site warmed on a resizable view installs an inline cache that also accepts
    // non-resizable views; an immutable one must not slip through it.
    let resizable = new Uint8Array(new ArrayBuffer(8, { maxByteLength: 16 }));
    let immutable = new Uint8Array(new ArrayBuffer(8).transferToImmutable());
    function store(v, i, x) { v[i] = x; }
    for (let i = 0; i < testLoopCount; ++i)
        store(resizable, 0, 1);
    store(immutable, 0, 42);
    assertEq(immutable[0], 0, "resizable store cache must reject immutable views");
}

{
    // Atomics mutators keep throwing once the call site is JIT-compiled.
    let mutable = new Int32Array(new ArrayBuffer(32));
    let immutable = new Int32Array(new ArrayBuffer(32).transferToImmutable());
    function atomicAdd(v) { return Atomics.add(v, 0, 1); }
    function atomicStore(v) { return Atomics.store(v, 0, 7); }
    for (let i = 0; i < testLoopCount; ++i) {
        atomicAdd(mutable);
        atomicStore(mutable);
    }
    assertThrows(() => atomicAdd(immutable), TypeError, "Atomics.add throws after tier up");
    assertThrows(() => atomicStore(immutable), TypeError, "Atomics.store throws after tier up");
    assertEq(immutable[0], 0, "Atomics never wrote to the immutable buffer");
}

{
    // The per-type Structure for immutable views must stay reachable across collections.
    for (let i = 0; i < 100; ++i) {
        let view = new Uint8Array(new ArrayBuffer(8).transferToImmutable());
        view = null;
        gc();
    }
    assertEq(new Uint8Array(new ArrayBuffer(4).transferToImmutable()).length, 4, "immutable Structure outlives GC");
    assertEq(new DataView(new ArrayBuffer(4).transferToImmutable()).byteLength, 4, "immutable DataView Structure outlives GC");
}
