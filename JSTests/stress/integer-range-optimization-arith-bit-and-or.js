...
// Originally added in 312345@main (constant-operand only), reverted in 312458@main
// for a delta-blue regression, and relanded with the rangeFor()-based form that
// also narrows from non-constant operands whose range is already known (e.g. the
// result of another mask). These are correctness assertions: a wrong bound on the
// result node would let a later CheckInBounds elide unsoundly and read out of bounds,
// or fold a comparison the wrong way.
...
// This is the case the relanded rangeFor() form handles that the constant-only
// version could not: neither operand of the outer `&` is a constant.
