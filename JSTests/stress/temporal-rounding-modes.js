...
// Exact halfway: 12:30:00 rounded to hour
{
...
// --- Duration.round with each rounding mode ---
{
...
// --- Negative duration rounding (sign-aware) ---
{
    const dur = Temporal.Duration.from({ hours: -5, minutes: -30 });
    const expected = {
...
// --- Instant.round with each mode ---
{
    ...
    for (const mode of modes) {
        ...
        // Verify it produces a valid result (no crash)
        const h = new Temporal.ZonedDateTime(result.epochNanoseconds, "UTC").hour;
        shouldBe(h === 12 || h === 13, true, `Instant round hour ${mode} valid`);
    }
}
