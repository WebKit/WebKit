...
// Regression tests for ZonedDateTime.with() at epoch boundary cases.
//
// Bug fixed: ZonedDateTime.with() in the Prefer/Reject offset branch called
// getPossibleEpochNanosecondsFor() directly without first checking CheckISODaysRange
// (spec InterpretISODateTimeOffset step 7). When the resulting local date after field
// merging falls outside ±10^8 days, it must throw RangeError — even if the UTC epoch
// would technically be valid.
//
// Root cause: minEpoch in timezone -12:34 projects to local date -271821-04-19
// (day -100000001, outside ±10^8). Any with() that keeps this local date must throw.
...
// Any with() that results in this date must throw.
{
    ...
    // Actually 'use' bypasses CheckISODaysRange in our implementation...
    // Just verify 'prefer' and 'reject' throw (the cases tested by benchmark)
}
...
// === Normal with() behavior unaffected ===
{
