// Standalone JetStream3 CDJS/cdjs workload (collision detection; Purdue/Apple, BSD). The whole
// subtest is inlined verbatim (Constants, RedBlackTree, Vector2D/3D, Motion, CollisionDetector,
// Simulator, benchmark) and driven by the real benchmark() (1000 aircraft x 18 frames, which
// asserts the expected collision count). cdjs is FTL-dominated; its hot path is the RedBlackTree
// machinery (integer-valued Vector2D voxel comparisons via compareNumbers) plus findIntersection.
//
// FINDING (Support unsigned in IntegerRangeOptimization): the JS3 cdjs regression is a COMPILE-TIME
// regression, not a steady-state one. Timed via run-jsc-benchmarks this file is FLAT (the JS3
// "Average"/steady-state component is unaffected, ~+0.2% n.s.). The cost is in compilation: run
//   DYLD_FRAMEWORK_PATH=<dir> <dir>/jsc --reportFTLCompileTimes=true <thisfile>
// and sum the per-function DFG phase times -- patched is ~+9.5% (FTL total ~+5.5%) because the
// expanded IntegerRangeOptimization phase costs more to run, which delays FTL tier-up and inflates
// the JS3 First-iteration (+6.5%) and Worst-case (+4.0%) times that the cdjs score weights.
...
// ===== driver: loop the real cdjs benchmark() (deterministic; asserts 1336 collisions) =====
noInline(reduceCollisionSet);
let sink = 0;
for (let k = 0; k < 20; k++) {
    benchmark();
    sink = (sink + 1) | 0;
