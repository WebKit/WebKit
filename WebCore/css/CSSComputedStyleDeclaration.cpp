...
static PassRefPtr<CSSValue> getBorderRadiusCornerValue(IntSize radius, const RenderStyle* style)
{
    if (radius.width() == radius.height())
        return CSSPrimitiveValue::create(radius.width(), CSSPrimitiveValue::CSS_PX);

    RefPtr<CSSValueList> list = CSSValueList::createSpaceSeparated();
    list->append(zoomAdjustedPixelValue(radius.width(), style));
    list->append(zoomAdjustedPixelValue(radius.height(), style));
    return list.release();
}
