
// memory_trap.wast:1
let $1 = instance("");

// memory_trap.wast:21
assert_return(() => call($1, "store", [-4, 42]));

// memory_trap.wast:22
assert_return(() => call($1, "load", [-4]), 42);

// memory_trap.wast:23
assert_trap(() => call($1, "store", [-3, 13]));

// memory_trap.wast:24
assert_trap(() => call($1, "load", [-3]));

// memory_trap.wast:25
assert_trap(() => call($1, "store", [-2, 13]));

// memory_trap.wast:26
assert_trap(() => call($1, "load", [-2]));

// memory_trap.wast:27
assert_trap(() => call($1, "store", [-1, 13]));

// memory_trap.wast:28
assert_trap(() => call($1, "load", [-1]));

// memory_trap.wast:29
assert_trap(() => call($1, "store", [0, 13]));

// memory_trap.wast:30
assert_trap(() => call($1, "load", [0]));

// memory_trap.wast:31
assert_trap(() => call($1, "store", [-2147483648, 13]));

// memory_trap.wast:32
assert_trap(() => call($1, "load", [-2147483648]));

// memory_trap.wast:33
assert_return(() => call($1, "grow_memory", [65537]), -1);

// memory_trap.wast:35
let $2 = instance("");

// memory_trap.wast:111
assert_trap(() => call($2, "i32.store", [65536, 0]));

// memory_trap.wast:112
assert_trap(() => call($2, "i32.store", [65535, 0]));

// memory_trap.wast:113
assert_trap(() => call($2, "i32.store", [65534, 0]));

// memory_trap.wast:114
assert_trap(() => call($2, "i32.store", [65533, 0]));

// memory_trap.wast:115
assert_trap(() => call($2, "i32.store", [-1, 0]));

// memory_trap.wast:116
assert_trap(() => call($2, "i32.store", [-2, 0]));

// memory_trap.wast:117
assert_trap(() => call($2, "i32.store", [-3, 0]));

// memory_trap.wast:118
assert_trap(() => call($2, "i32.store", [-4, 0]));

// memory_trap.wast:119
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [65536, int64("0")]))

// memory_trap.wast:120
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [65535, int64("0")]))

// memory_trap.wast:121
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [65534, int64("0")]))

// memory_trap.wast:122
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [65533, int64("0")]))

// memory_trap.wast:123
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [65532, int64("0")]))

// memory_trap.wast:124
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [65531, int64("0")]))

// memory_trap.wast:125
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [65530, int64("0")]))

// memory_trap.wast:126
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [65529, int64("0")]))

// memory_trap.wast:127
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [-1, int64("0")]))

// memory_trap.wast:128
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [-2, int64("0")]))

// memory_trap.wast:129
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [-3, int64("0")]))

// memory_trap.wast:130
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [-4, int64("0")]))

// memory_trap.wast:131
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [-5, int64("0")]))

// memory_trap.wast:132
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [-6, int64("0")]))

// memory_trap.wast:133
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [-7, int64("0")]))

// memory_trap.wast:134
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store", [-8, int64("0")]))

// memory_trap.wast:135
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.store", [65536, 0.]))

// memory_trap.wast:136
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.store", [65535, 0.]))

// memory_trap.wast:137
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.store", [65534, 0.]))

// memory_trap.wast:138
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.store", [65533, 0.]))

// memory_trap.wast:139
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.store", [-1, 0.]))

// memory_trap.wast:140
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.store", [-2, 0.]))

// memory_trap.wast:141
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.store", [-3, 0.]))

// memory_trap.wast:142
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.store", [-4, 0.]))

// memory_trap.wast:143
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [65536, 0.]))

// memory_trap.wast:144
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [65535, 0.]))

// memory_trap.wast:145
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [65534, 0.]))

// memory_trap.wast:146
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [65533, 0.]))

// memory_trap.wast:147
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [65532, 0.]))

// memory_trap.wast:148
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [65531, 0.]))

// memory_trap.wast:149
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [65530, 0.]))

// memory_trap.wast:150
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [65529, 0.]))

// memory_trap.wast:151
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [-1, 0.]))

// memory_trap.wast:152
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [-2, 0.]))

// memory_trap.wast:153
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [-3, 0.]))

// memory_trap.wast:154
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [-4, 0.]))

// memory_trap.wast:155
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [-5, 0.]))

// memory_trap.wast:156
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [-6, 0.]))

// memory_trap.wast:157
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [-7, 0.]))

// memory_trap.wast:158
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.store", [-8, 0.]))

// memory_trap.wast:159
assert_trap(() => call($2, "i32.store8", [65536, 0]));

// memory_trap.wast:160
assert_trap(() => call($2, "i32.store8", [-1, 0]));

// memory_trap.wast:161
assert_trap(() => call($2, "i32.store16", [65536, 0]));

// memory_trap.wast:162
assert_trap(() => call($2, "i32.store16", [65535, 0]));

// memory_trap.wast:163
assert_trap(() => call($2, "i32.store16", [-1, 0]));

// memory_trap.wast:164
assert_trap(() => call($2, "i32.store16", [-2, 0]));

// memory_trap.wast:165
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store8", [65536, int64("0")]))

// memory_trap.wast:166
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store8", [-1, int64("0")]))

// memory_trap.wast:167
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store16", [65536, int64("0")]))

// memory_trap.wast:168
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store16", [65535, int64("0")]))

// memory_trap.wast:169
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store16", [-1, int64("0")]))

// memory_trap.wast:170
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store16", [-2, int64("0")]))

// memory_trap.wast:171
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store32", [65536, int64("0")]))

// memory_trap.wast:172
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store32", [65535, int64("0")]))

// memory_trap.wast:173
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store32", [65534, int64("0")]))

// memory_trap.wast:174
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store32", [65533, int64("0")]))

// memory_trap.wast:175
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store32", [-1, int64("0")]))

// memory_trap.wast:176
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store32", [-2, int64("0")]))

// memory_trap.wast:177
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store32", [-3, int64("0")]))

// memory_trap.wast:178
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.store32", [-4, int64("0")]))

// memory_trap.wast:179
assert_trap(() => call($2, "i32.load", [65536]));

// memory_trap.wast:180
assert_trap(() => call($2, "i32.load", [65535]));

// memory_trap.wast:181
assert_trap(() => call($2, "i32.load", [65534]));

// memory_trap.wast:182
assert_trap(() => call($2, "i32.load", [65533]));

// memory_trap.wast:183
assert_trap(() => call($2, "i32.load", [-1]));

// memory_trap.wast:184
assert_trap(() => call($2, "i32.load", [-2]));

// memory_trap.wast:185
assert_trap(() => call($2, "i32.load", [-3]));

// memory_trap.wast:186
assert_trap(() => call($2, "i32.load", [-4]));

// memory_trap.wast:187
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [65536]))

// memory_trap.wast:188
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [65535]))

// memory_trap.wast:189
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [65534]))

// memory_trap.wast:190
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [65533]))

// memory_trap.wast:191
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [65532]))

// memory_trap.wast:192
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [65531]))

// memory_trap.wast:193
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [65530]))

// memory_trap.wast:194
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [65529]))

// memory_trap.wast:195
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [-1]))

// memory_trap.wast:196
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [-2]))

// memory_trap.wast:197
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [-3]))

// memory_trap.wast:198
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [-4]))

// memory_trap.wast:199
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [-5]))

// memory_trap.wast:200
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [-6]))

// memory_trap.wast:201
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [-7]))

// memory_trap.wast:202
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load", [-8]))

// memory_trap.wast:203
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.load", [65536]))

// memory_trap.wast:204
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.load", [65535]))

// memory_trap.wast:205
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.load", [65534]))

// memory_trap.wast:206
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.load", [65533]))

// memory_trap.wast:207
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.load", [-1]))

// memory_trap.wast:208
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.load", [-2]))

// memory_trap.wast:209
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.load", [-3]))

// memory_trap.wast:210
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f32.load", [-4]))

// memory_trap.wast:211
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [65536]))

// memory_trap.wast:212
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [65535]))

// memory_trap.wast:213
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [65534]))

// memory_trap.wast:214
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [65533]))

// memory_trap.wast:215
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [65532]))

// memory_trap.wast:216
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [65531]))

// memory_trap.wast:217
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [65530]))

// memory_trap.wast:218
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [65529]))

// memory_trap.wast:219
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [-1]))

// memory_trap.wast:220
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [-2]))

// memory_trap.wast:221
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [-3]))

// memory_trap.wast:222
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [-4]))

// memory_trap.wast:223
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [-5]))

// memory_trap.wast:224
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [-6]))

// memory_trap.wast:225
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [-7]))

// memory_trap.wast:226
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "f64.load", [-8]))

// memory_trap.wast:227
assert_trap(() => call($2, "i32.load8_s", [65536]));

// memory_trap.wast:228
assert_trap(() => call($2, "i32.load8_s", [-1]));

// memory_trap.wast:229
assert_trap(() => call($2, "i32.load8_u", [65536]));

// memory_trap.wast:230
assert_trap(() => call($2, "i32.load8_u", [-1]));

// memory_trap.wast:231
assert_trap(() => call($2, "i32.load16_s", [65536]));

// memory_trap.wast:232
assert_trap(() => call($2, "i32.load16_s", [65535]));

// memory_trap.wast:233
assert_trap(() => call($2, "i32.load16_s", [-1]));

// memory_trap.wast:234
assert_trap(() => call($2, "i32.load16_s", [-2]));

// memory_trap.wast:235
assert_trap(() => call($2, "i32.load16_u", [65536]));

// memory_trap.wast:236
assert_trap(() => call($2, "i32.load16_u", [65535]));

// memory_trap.wast:237
assert_trap(() => call($2, "i32.load16_u", [-1]));

// memory_trap.wast:238
assert_trap(() => call($2, "i32.load16_u", [-2]));

// memory_trap.wast:239
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load8_s", [65536]))

// memory_trap.wast:240
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load8_s", [-1]))

// memory_trap.wast:241
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load8_u", [65536]))

// memory_trap.wast:242
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load8_u", [-1]))

// memory_trap.wast:243
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load16_s", [65536]))

// memory_trap.wast:244
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load16_s", [65535]))

// memory_trap.wast:245
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load16_s", [-1]))

// memory_trap.wast:246
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load16_s", [-2]))

// memory_trap.wast:247
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load16_u", [65536]))

// memory_trap.wast:248
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load16_u", [65535]))

// memory_trap.wast:249
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load16_u", [-1]))

// memory_trap.wast:250
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load16_u", [-2]))

// memory_trap.wast:251
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_s", [65536]))

// memory_trap.wast:252
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_s", [65535]))

// memory_trap.wast:253
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_s", [65534]))

// memory_trap.wast:254
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_s", [65533]))

// memory_trap.wast:255
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_s", [-1]))

// memory_trap.wast:256
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_s", [-2]))

// memory_trap.wast:257
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_s", [-3]))

// memory_trap.wast:258
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_s", [-4]))

// memory_trap.wast:259
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_u", [65536]))

// memory_trap.wast:260
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_u", [65535]))

// memory_trap.wast:261
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_u", [65534]))

// memory_trap.wast:262
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_u", [65533]))

// memory_trap.wast:263
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_u", [-1]))

// memory_trap.wast:264
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_u", [-2]))

// memory_trap.wast:265
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_u", [-3]))

// memory_trap.wast:266
assert_trap(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_trap(() => call($2, "i64.load32_u", [-4]))

// memory_trap.wast:269
run(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_return(() => call($2, "i64.load", [65528]), int64("7523094288207667809"))

// memory_trap.wast:270
run(() => call(instance("", exports("$2", $2)),  "run", []));  // assert_return(() => call($2, "i64.load", [0]), int64("7523094288207667809"))
