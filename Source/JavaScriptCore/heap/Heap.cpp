...
UnsafeHashMap<const char*, GCTypeMap<SimpleStats>>& timingStats()
{
    static UnsafeHashMap<const char*, GCTypeMap<SimpleStats>>* result;
    static std::once_flag once;
    std::call_once(
        once,
        [] {
            result = new UnsafeHashMap<const char*, GCTypeMap<SimpleStats>>();
        });
    return *result;
}
...
NEVER_INLINE bool Heap::runFixpointPhase(GCConductor conn)
{
    ...
    if (UNLIKELY(Options::logGC())) {
        UnsafeHashMap<const char*, size_t> visitMap;
        forEachSlotVisitor(
            [&] (SlotVisitor& visitor) {
