    ...
    PrintStream& out, DumpContext* context, Structure* structure) const
{
    if (!*this)
        ...
        if (structure->classInfoForCells()->isSubClassOf(JSString::info())) {
            ...
            out.print("RegExp: ", *jsUncheckedDowncast<RegExp*>(asCell()));
            ...
            out.print("Structure: ", inContext(*jsUncheckedDowncast<Structure*>(asCell()), context));
        else if (isHeapBigInt())
            out.print("BigInt[heap-allocated]: addr=", RawPointer(asCell()), ", length=", jsUncheckedDowncast<JSBigInt*>(asCell())->length(), ", sign=", jsUncheckedDowncast<JSBigInt*>(asCell())->sign());
...
WTF::String JSValue::toWTFStringForConsole(JSGlobalObject* globalObject) const
{
    ...
    if (jsDynamicDowncast<JSArray*>(*this))
        return tryMakeString('[', result.data, ']');
    if (jsDynamicDowncast<JSBigInt*>(*this))
