...
#define FOR_EACH_HASH_TABLE_VALUE_TYPE(v) \
    v(Accessor) \
    v(BuiltinAccessor) \
    v(BuiltinGenerator) \
    v(Constant) \
...
#define DECLARE_TYPE(name) name,
        FOR_EACH_HASH_TABLE_VALUE_TYPE(DECLARE_TYPE)
        End,
#undef DECLARE_TYPE
    };
...
#define DECLARE_TAG(name) enum name##TypeTag { name##Type };
    FOR_EACH_HASH_TABLE_VALUE_TYPE(DECLARE_TAG)
    enum EndTag { End };
#undef DECLARE_TAG
...
inline void reifyStaticProperty(VM& vm, const ClassInfo* classInfo, const PropertyName& propertyName, const HashTableValue& value, JSObject& thisObj)
{
    ...
    if (value.attributes() & PropertyAttribute::ClassStructure) {
        ...
        JSObject* constructor = lazyStructure->constructor(jsUncheckedDowncast<JSGlobalObject*>(&thisObj));
