...
typedef bool (*IsLazyPropertyEnabledCallback)(JSGlobalObject*);
...
struct HashTableValue {
    ...
    bool isLazyPropertyEnabled(JSGlobalObject* globalObject) const
    {
...
JS_EXPORT_PRIVATE void setUpStaticPropertySlot(VM&, const ClassInfo*, const HashTableValue*, JSObject* thisObject, PropertyName, PropertySlot&);
...
// FIXME: Inline this function into its call site so staticPropertiesReified() check can be hoisted.
inline bool getStaticPropertySlotFromTable(VM& vm, const ClassInfo* classInfo, const HashTable& table, JSObject* thisObject, PropertyName propertyName, PropertySlot& slot)
{
    ...
    if ((entry->attributes() & PropertyAttribute::PropertyCallback) && !entry->isLazyPropertyEnabled(thisObject->globalObject(vm))) {
        slot.disableCaching();
    ...
    if (entry->attributes() & PropertyAttribute::DOMJITAttribute) {
        ...
        slot.setCacheableCustom(thisObject, attributesForStructure(entry->attributes()), domJIT->getter(), entry->propertyPutter(), DOMAttributeAnnotation { classInfo, domJIT });
        return true;
    }
...
inline void reifyStaticProperty(VM& vm, const ClassInfo* classInfo, const PropertyName& propertyName, const HashTableValue& value, JSObject& thisObj)
{
    ...
    if (value.attributes() & PropertyAttribute::PropertyCallback) {
        ASSERT(value.isLazyPropertyEnabled(thisObj.globalObject(vm)));
...
{
    JSGlobalObject* globalObject = thisObj.globalObject(vm);
    BatchedTransitionOptimizer transitionOptimizer(vm, &thisObj);
    for (auto& value : values) {
        if (!value.m_key)
            continue;
        if ((value.attributes() & PropertyAttribute::PropertyCallback) && !value.isLazyPropertyEnabled(globalObject))
            continue;
