    ...
    , m_globalObject(structure->realm(), WriteBarrierEarlyInit)
{
    ...
    ASSERT(jsDynamicDowncast<InternalFunction*>(this));
...
{
    InternalFunction* thisObject = jsUncheckedDowncast<InternalFunction*>(cell);
...
CallData InternalFunction::getCallData(JSCell* cell)
{
    ...
    auto* function = jsUncheckedDowncast<InternalFunction*>(cell);
...
Structure* InternalFunction::createSubclassStructure(JSGlobalObject* globalObject, JSObject* newTarget, Structure* baseClass)
{
    ...
    JSFunction* targetFunction = jsDynamicDowncast<JSFunction*>(newTarget);
    ...
    if (!targetFunction || !targetFunction->canUseAllocationProfiles()) [[unlikely]] {
        ...
        if (JSObject* prototype = jsDynamicDowncast<JSObject*>(prototypeValue)) {
...
JSGlobalObject* getFunctionRealm(JSGlobalObject* globalObject, JSObject* object)
{
    ...
    while (true) {
        if (object->inherits<JSBoundFunction>()) {
            object = jsUncheckedDowncast<JSBoundFunction*>(object)->targetFunction();
            continue;
        }
        if (object->inherits<JSRemoteFunction>()) {
            object = jsUncheckedDowncast<JSRemoteFunction*>(object)->targetFunction();
            continue;
        }

        if (object->type() == ProxyObjectType) {
            auto* proxy = jsUncheckedDowncast<ProxyObject*>(object);
            if (proxy->isRevoked()) {
