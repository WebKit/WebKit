...
public:
    ...
    {
        return StringHasher::finalizeAndMaskTop8Bits(processPendingCharacter());
    }
    ...
    template<typename T, typename Converter = DefaultConverter>
    static constexpr unsigned computeHashAndMaskTop8Bits(const T* data, unsigned length)
    {
        return StringHasher::finalizeAndMaskTop8Bits(computeHashImpl<T, Converter>(data, length));
    }

    template<typename T, typename Converter = DefaultConverter>
    static constexpr unsigned computeHashAndMaskTop8Bits(const T* data)
    {
