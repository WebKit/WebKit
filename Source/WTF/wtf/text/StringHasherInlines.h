...
constexpr unsigned StringHasher::computeHashAndMaskTop8Bits(const T* data, unsigned characterCount)
{
...
inline unsigned StringHasher::hashWithTop8BitsMasked()
{
    ...
    if (!m_pendingHashValue) {
        ...
        hashValue = StringHasher::finalizeAndMaskTop8Bits(WYHash::handleEndCase(a, b, m_seed, totalByteCount));
