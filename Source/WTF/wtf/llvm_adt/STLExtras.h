...
#include <wtf/llvm_adt/iterator.h>
...
// Sections of this file that are unwanted for WTF should be put behind this macro.
#define DISABLED_FROM_LLVM 0
...
detail::zippy<detail::zip_first, T, U, Args...> zip_equal(T &&t, U &&u,
                                                          Args &&...args) {
  assert(detail::all_equal({range_size(t), range_size(u), range_size(args)...}) &&
...
class concat_iterator
  ...
  template <size_t... Ns> void increment(std::index_sequence<Ns...>) {
    ...
    RELEASE_ASSERT_NOT_REACHED("Attempted to increment an end concat iterator!");
  }
  ...
  template <size_t... Ns> ValueT &get(std::index_sequence<Ns...>) const {
    ...
    RELEASE_ASSERT_NOT_REACHED("Attempted to get a pointer from an end concat iterator!");
  }
...
class indexed_accessor_iterator
    : public WTF::iterator_facade_base<DerivedT,
...
template <typename ContainerTy> auto make_first_range(ContainerTy &&c) {
  ...
  return WTF::map_range(std::forward<ContainerTy>(c),
...
struct index_iterator
    : WTF::iterator_facade_base<index_iterator,
...
auto enumerate(FirstRange &&First, RestRanges &&...Rest) {
  if constexpr (sizeof...(Rest) != 0) {
    ...
    assert(detail::all_equal(sizes) && "Ranges have different length");
#endif
  }
...
struct tuple_size<WTF::detail::enumerator_result<Refs...>>
    : std::integral_constant<std::size_t, sizeof...(Refs)> {};
 
template <std::size_t I, typename... Refs>
struct tuple_element<I, WTF::detail::enumerator_result<Refs...>>
    : std::tuple_element<I, std::tuple<Refs...>> {};
 
template <std::size_t I, typename... Refs>
struct tuple_element<I, const WTF::detail::enumerator_result<Refs...>>
    : std::tuple_element<I, std::tuple<Refs...>> {};
