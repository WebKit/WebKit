...
#if defined(QP_SUPPORT_PNG_IMAGEIO)
                             ...
                             const void *data)
{
    ...
    CGImageRef image;
    CFMutableDataRef pngData;
    CGImageDestinationRef dest;

    DE_ASSERT(imageFormat == QP_IMAGE_FORMAT_RGB888 || imageFormat == QP_IMAGE_FORMAT_RGBA8888);

    provider = CGDataProviderCreateWithData(NULL, data, (size_t)rowStride * (size_t)height, NULL);
    if (!provider)
        return false;

    colorSpace = CGColorSpaceCreateDeviceRGB();
    /* Describe the pixels as-is (straight alpha, byte order R,G,B[,A]). */
    image = CGImageCreate((size_t)width, (size_t)height, 8, bytesPerPixel * 8, (size_t)rowStride, colorSpace,
                          hasAlpha ? (kCGImageAlphaLast | kCGBitmapByteOrderDefault)
    ...
    pngData = CFDataCreateMutable(NULL, 0);
    dest    = pngData ? CGImageDestinationCreateWithData(pngData, CFSTR("public.png"), 1, NULL) : NULL;
    if (dest)
    {
        CGImageDestinationAddImage(dest, image, NULL);
        if (CGImageDestinationFinalize(dest))
            compressOk = Buffer_append(buffer, CFDataGetBytePtr(pngData), (size_t)CFDataGetLength(pngData));
        CFRelease(dest);
    }
