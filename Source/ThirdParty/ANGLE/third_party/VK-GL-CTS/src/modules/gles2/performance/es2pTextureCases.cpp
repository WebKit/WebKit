...
Texture2DRenderCase::Texture2DRenderCase(Context &context, const char *name, const char *description, uint32_t format,
                                         uint32_t dataType, uint32_t wrapS, uint32_t wrapT, uint32_t minFilter,
...
void Texture2DRenderCase::init(void)
{
    ...
    log << TestLog::Message << "Format: " << glu::getTextureFormatName(m_format) << " " << glu::getTypeName(m_dataType)
        << TestLog::EndMessage;
    ...
    for (int texNdx = 0; texNdx < m_numTextures; texNdx++)
        fragSrc << "\t" << (texNdx == 0 ? "lowp vec4 r = " : "r += ") << "texture2D(u_sampler" << texNdx
                << ", v_coords);\n";
