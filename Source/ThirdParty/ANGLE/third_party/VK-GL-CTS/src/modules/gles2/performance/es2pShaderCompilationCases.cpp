                                     ...
                                     ShaderValidity validity)
{
    ...
    params["SEMANTIC_ERROR"] =
        validity != SHADER_VALIDITY_SEMANTIC_ERROR ? "" : "\tmediump float invalid = sin(1.0, 2.0);\n";
    params["INVALID_CHAR"] =
...
static string lightVertexTemplate(int numLights, bool isVertexCase, LightType lightType)
{
    ...
    resultTemplate += "attribute highp vec4 a_position${NAME_SPEC};\n"
    ...
    if (!isVertexCase)
    {
        ...
        if (lightType == LIGHT_POINT)
            resultTemplate += "varying mediump vec3 v_directionToLight${NAME_SPEC}[" + de::toString(numLights) +
                              "];\n"
...
static string lightFragmentTemplate(int numLights, bool isVertexCase, LightType lightType)
{
    ...
    resultTemplate += "    color *= texture2D(u_sampler0${NAME_SPEC}, texCoord0);\n"
...
static string loopVertexTemplate(LoopType type, bool isVertexCase, int numLoopIterations, int nestingDepth)
{
    ...
    if (type == LOOP_TYPE_DYNAMIC)
        resultTemplate += "attribute mediump float a_loopBound${NAME_SPEC};\n";

    resultTemplate += "attribute mediump vec4 a_value${NAME_SPEC};\n"
    ...
    {
        if (type == LOOP_TYPE_DYNAMIC)
            resultTemplate += "varying mediump float v_loopBound${NAME_SPEC};\n";
...
static string loopFragmentTemplate(LoopType type, bool isVertexCase, int numLoopIterations, int nestingDepth)
{
    ...
    resultTemplate += "varying mediump vec4 v_value${NAME_SPEC};\n";
    ...
    if (!isVertexCase)
    {
        ...
        resultTemplate += "    gl_FragColor = value + ${FLOAT01};\n";
    }
    else
        resultTemplate += "\n"
                          "void main()\n"
...
{
    const char *resultTemplate = "attribute highp vec4 a_position${NAME_SPEC};\n"
                                 "attribute mediump vec4 a_value${NAME_SPEC};\n"
                                 "varying mediump vec4 v_value${NAME_SPEC};\n"
                                 "\n"
                                 "void main()\n"
...
{
    const char *resultTemplate = "varying mediump vec4 v_value${NAME_SPEC};\n"
                                 "\n"
                                 "void main()\n"
                                 "{\n"
                                 "    gl_FragColor = v_value${NAME_SPEC} + ${FLOAT01};\n"
...
static string textureLookupVertexTemplate(ConditionalUsage conditionalUsage, ConditionalType conditionalType)
{
    ...
    if (conditionVaryingNeeded)
        resultTemplate += "attribute mediump float a_condition${NAME_SPEC};\n"
                                            ...
                                            ConditionalType conditionalType)
{
    ...
    resultTemplate += "varying mediump vec2 v_coords${NAME_SPEC};\n";

    if (conditionalUsage != CONDITIONAL_USAGE_NONE && conditionalType == CONDITIONAL_TYPE_DYNAMIC)
        resultTemplate += "varying mediump float v_condition${NAME_SPEC};\n";
    ...
    resultTemplate += "    gl_FragColor = color/" + de::toString(numLookups) + ".0 + ${FLOAT01};\n" +
                      "${SEMANTIC_ERROR}"
                      "}\n"
                      "${INVALID_CHAR}";
...
{
    string resultTemplate = "varying mediump vec2 v_coord${NAME_SPEC};\n"
                            "\n"
