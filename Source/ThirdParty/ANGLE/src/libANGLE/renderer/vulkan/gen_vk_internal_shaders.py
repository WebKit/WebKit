...
template_spirv_blob_inc = """// GENERATED FILE - DO NOT EDIT.
...
def gen_shader_blob_entry(shader):
    ...
    return "{{{}, {}}}".format(var_name, "sizeof(%s)" % var_name)
...
def write_compressed_spirv_blob_as_c_array(output_path, variable_name, compressed_blob,
                                           preprocessed_source):
    hex_array = [f'0x{byte:02x}' for byte in compressed_blob]
...
def compile_variation(glslang_path, compile_queue, shader_file, shader_basename, flags, enums,
    ...
    output_name = '{}.{:08X}'.format(shader_basename, variation_bits)
...
def get_variation_definition(shader_and_variation):
    ...
    if len(flags) > 0:
        ...
        definition += ''.join([
            'k{} = 0x{:08X},\n'.format(get_variation_name(flags[f]), 1 << f) for f in range(len(flags))
        ])
        definition += '};\n'
    ...
    for e in range(len(enums)):
        ...
        definition += 'enum %s\n{\n' % enum_name
        definition += ''.join([
            'k{} = 0x{:08X},\n'.format(get_variation_name(enum[1][v]), v << current_bit_start)
...
def get_shader_table_cpp(shader_and_variation):
    ...
    for variation in range(array_len):
        ...
        if any([(variation & enum_masks[e]) > enum_maxes[e] for e in range(len(enums))]):
            ...
            entry = '{}_{:08X}'.format(var_name, variation)
            table += '{{{}, sizeof({})}},\n'.format(entry, entry)
...
def get_get_function_cpp(shader_and_variation):
    ...
    definition += '(Context *context, uint32_t shaderFlags, RefCounted<ShaderModule> **shaderOut)\n{\n'
    definition += 'return GetShader(context, {}, {}, ArraySize({}), shaderFlags, shaderOut);\n}}\n'.format(
...
def main():
    ...
    with open(out_file_gni, 'w', newline='\n') as outfile:
