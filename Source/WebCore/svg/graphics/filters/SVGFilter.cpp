...
std::optional<std::tuple<SVGFilterExpression, FilterEffectVector>> SVGFilter::buildExpression(SVGFilterElement& filterElement, const SVGFilter& filter, const GraphicsContext& destinationContext)
{
    ...
    bool success = graph.visit([&](FilterEffect& effect, unsigned level) {
        ...
        expression.append({ static_cast<unsigned>(index), effectGeometry(effect), level });
    });
...
FilterResults& SVGFilter::ensureResults(const Function<std::unique_ptr<FilterResults>()>& callback)
{
    if (!m_results)
        m_results = callback();
    return *m_results;
...
void SVGFilter::addToTransaction(FilterEffect& effect)
{
    m_transactionEffects.add(Ref { effect });
    if (m_results)
        m_results->clearEffectResult(effect);
}

SVGFilterTransaction SVGFilter::buildTransaction()
{
    SVGFilterTransaction transaction;

    for (auto& effect : m_transactionEffects) {
        auto index = m_effects.findIf([&](auto& item) {
            return item.ptr() == effect.ptr();
        });
        ASSERT(index != notFound);
        transaction.append({ static_cast<unsigned>(index), Ref { effect } });
    }

    m_transactionEffects.clear();
    return transaction;
}

void SVGFilter::applyTransaction(const SVGFilterTransaction& transaction)
{
    for (auto& item : transaction) {
        if (item.index >= m_effects.size()) {
            ASSERT_NOT_REACHED();
            return;
        }

        auto& effect = m_effects[item.index];

        if (m_results)
            m_results->clearEffectResult(effect);

        if (effect->filterType() != item.effect->filterType()) {
            ASSERT_NOT_REACHED();
            return;
        }
