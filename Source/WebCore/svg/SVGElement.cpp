...
void SVGElement::synchronizeAttribute(const QualifiedName& name)
{
    ...
    if (auto value = propertyRegistry().synchronize(name)) {
        auto& targetName = attributeNameForSynchronization(name);
        ...
        if (value->isEmpty() && !hasStoredAttribute(targetName))
            return;
...
void SVGElement::synchronizeAllAttributes()
{
    ...
    for (const auto& entry : map) {
        auto& targetName = attributeNameForSynchronization(entry.key);
        setSynchronizedLazyAttribute(targetName, AtomString { entry.value });
    }
}

bool SVGElement::hasStoredAttribute(const QualifiedName& name) const
{
...
{
    if (attributeName.matches(SVGNames::hrefAttr) || attributeName.matches(XLinkNames::hrefAttr)) [[unlikely]] {
