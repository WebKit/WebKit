    ...
    while (true) {
        ...
        if (cs->match() == Match::Id) {
            ...
            case PseudoClass::Lang:
                ...
                appendCommaSeparatedPossiblyQuotedIdentifierList(builder, *cs->argumentList());
                builder.append(')');
                break;
            ...
            case PseudoClass::ActiveViewTransitionType:
                builder.append('(');
                ASSERT_WITH_MESSAGE(cs->argumentList() && !cs->argumentList()->isEmpty(), "An empty :active-view-transition-type() is invalid and should never be generated by the parser.");
                appendCommaSeparatedPossiblyQuotedIdentifierList(builder, *cs->argumentList());
                builder.append(')');
                break;
            default:
