...
LocalFrame* LocalFrame::fromJSContext(JSContextRef context)
{
    ...
    if (auto* window = JSC::jsDynamicDowncast<JSDOMWindow*>(globalObjectObj))
        return dynamicDowncast<LocalFrame>(window->wrapped().frame());
    if (auto* serviceWorkerGlobalScope = JSC::jsDynamicDowncast<JSServiceWorkerGlobalScope*>(globalObjectObj))
...
LocalFrame* LocalFrame::contentFrameFromWindowOrFrameElement(JSContextRef context, JSValueRef valueRef)
{
    ...
    auto* jsNode = JSC::jsDynamicDowncast<JSNode*>(value);
    if (!jsNode)
        return nullptr;
