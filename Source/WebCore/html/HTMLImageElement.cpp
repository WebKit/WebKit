...
unsigned HTMLImageElement::width()
{
    ...
    if (!renderer()) {
        ...
        if (RefPtr image = m_imageLoader->image())
            return image->imageSizeForRenderer(nullptr, 1.0f, ImageSizeType::Intrinsic).width().toUnsigned();
    }
...
unsigned HTMLImageElement::height()
{
    ...
    if (!renderer()) {
        ...
        if (RefPtr image = m_imageLoader->image())
            return image->imageSizeForRenderer(nullptr, 1.0f, ImageSizeType::Intrinsic).height().toUnsigned();
    }
