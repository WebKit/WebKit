...
{
    if (belongsToBaseAppearanceListBox() && !tabIndexSetExplicitly())
        return false;
...
bool HTMLOptionElement::belongsToBaseAppearanceListBox() const
{
    RefPtr select = ownerSelectElement();
    return select && select->isBaseListBox();
}
