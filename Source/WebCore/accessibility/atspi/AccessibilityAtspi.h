...
private:
    ...
    UnsafeHashMap<CString, Vector<GUniquePtr<char*>>> m_eventListeners;
    UnsafeHashMap<AccessibilityRootAtspi*, Vector<unsigned, 3>> m_rootObjects;
    UnsafeHashMap<AccessibilityObjectAtspi*, Vector<unsigned, 7>> m_atspiObjects;
    UnsafeHashMap<AccessibilityObjectAtspi*, Vector<unsigned, 1>> m_atspiHyperlinks;
    UnsafeHashMap<CString, unsigned> m_clients;
    unsigned m_cacheID { 0 };
    UnsafeHashMap<String, AccessibilityObjectAtspi*> m_cache;
...
#if ENABLE(DEVELOPER_MODE)
    UnsafeHashMap<void*, NotificationObserver> m_notificationObservers;
#endif
};
