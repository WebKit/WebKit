...
static bool imageHasNothingToDraw(const RefPtr<Image>& image)
{
...
void CachedImage::didAddClient(CachedResourceClient& client)
{
    ...
    if (m_image && !imageHasNothingToDraw(m_image))
...
void CachedImage::updateBufferInternal(const FragmentedSharedBuffer& data)
{
    ...
    if (encodedDataStatus == EncodedDataStatus::Error || imageHasNothingToDraw(m_image)) {
