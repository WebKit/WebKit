...
static bool decodedToNothing(const RefPtr<Image>& image)
{
    return image && hasNothingToDraw(*image);
}
...
void CachedImage::didAddClient(CachedResourceClient& client)
{
    ...
    if (m_image && !decodedToNothing(m_image))
...
bool CachedImage::canRender(ImageOrientation orientation, float multiplier)
{
    if (errorOccurred())
        return false;
    ...
    if (!image)
        return false;
    auto size = selfReportedSize(*image, orientation);
...
void CachedImage::updateBufferInternal(const FragmentedSharedBuffer& data)
{
    ...
    if (encodedDataStatus == EncodedDataStatus::Error || decodedToNothing(m_image)) {
...
bool CachedImage::currentFrameKnownToBeOpaque()
{
    RefPtr image = this->image();
    return image->currentFrameKnownToBeOpaque();
}

bool CachedImage::currentFrameIsComplete()
{
    RefPtr image = this->image();
