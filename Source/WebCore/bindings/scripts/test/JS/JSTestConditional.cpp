...
{
    return getDOMConstructor<JSTestConditionalDOMConstructor, DOMConstructorID::TestConditional>(vm, *jsUncheckedDowncast<const JSDOMGlobalObject*>(globalObject));
}
...
JSC_DEFINE_CUSTOM_GETTER(jsTestConditionalConstructor, (JSGlobalObject* lexicalGlobalObject, EncodedJSValue thisValue, PropertyName))
{
    ...
    auto* prototype = jsDynamicDowncast<JSTestConditionalPrototype*>(JSValue::decode(thisValue));
...
{
    auto* thisObject = jsUncheckedDowncast<JSTestConditional*>(cell);
...
{
    if (auto* wrapper = jsDynamicDowncast<JSTestConditional*>(value))
