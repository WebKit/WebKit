...
{
    return getDOMConstructor<JSTestSetLikeDOMConstructor, DOMConstructorID::TestSetLike>(vm, *jsUncheckedDowncast<const JSDOMGlobalObject*>(globalObject));
}
...
JSC_DEFINE_CUSTOM_GETTER(jsTestSetLikeConstructor, (JSGlobalObject* lexicalGlobalObject, EncodedJSValue thisValue, PropertyName))
{
    ...
    auto* prototype = jsDynamicDowncast<JSTestSetLikePrototype*>(JSValue::decode(thisValue));
...
{
    auto* thisObject = jsUncheckedDowncast<JSTestSetLike*>(cell);
...
{
    if (auto* wrapper = jsDynamicDowncast<JSTestSetLike*>(value))
