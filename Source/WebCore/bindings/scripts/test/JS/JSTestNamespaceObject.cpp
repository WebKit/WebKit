...
#if ENABLE(Condition1)
    if (!jsUncheckedDowncast<JSDOMGlobalObject*>(&globalObject)->scriptExecutionContext()->settingsValues().testSetting1Enabled) {
...
#endif
    if (!jsUncheckedDowncast<JSDOMGlobalObject*>(&globalObject)->scriptExecutionContext()->settingsValues().testSetting2Enabled) {
...
{
    return getDOMConstructor<JSTestNamespaceObjectDOMConstructor, DOMConstructorID::TestNamespaceObject>(vm, *jsUncheckedDowncast<const JSDOMGlobalObject*>(globalObject));
}
