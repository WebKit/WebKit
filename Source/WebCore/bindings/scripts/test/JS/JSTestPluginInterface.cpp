...
{
    return getDOMConstructor<JSTestPluginInterfaceDOMConstructor, DOMConstructorID::TestPluginInterface>(vm, *jsUncheckedDowncast<const JSDOMGlobalObject*>(globalObject));
}
...
bool JSTestPluginInterface::legacyPlatformObjectGetOwnProperty(JSObject* object, JSGlobalObject* lexicalGlobalObject, PropertyName propertyName, PropertySlot& slot, bool ignoreNamedProperties)
{
    ...
    auto* thisObject = jsUncheckedDowncast<JSTestPluginInterface*>(object);
...
{
    auto* thisObject = jsUncheckedDowncast<JSTestPluginInterface*>(cell);
    ASSERT_GC_OBJECT_INHERITS(thisObject, info());
...
JSC_DEFINE_CUSTOM_GETTER(jsTestPluginInterfaceConstructor, (JSGlobalObject* lexicalGlobalObject, EncodedJSValue thisValue, PropertyName))
{
    SUPPRESS_UNCOUNTED_LOCAL auto& vm = JSC::getVM(lexicalGlobalObject);
    auto throwScope = DECLARE_THROW_SCOPE(vm);
    auto* prototype = jsDynamicDowncast<JSTestPluginInterfacePrototype*>(JSValue::decode(thisValue));
...
{
    if (auto* wrapper = jsDynamicDowncast<JSTestPluginInterface*>(value))
