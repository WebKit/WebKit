...
#if ENABLE(Condition22) || ENABLE(Condition23)
    if (!downcast<Document>(jsUncheckedDowncast<JSDOMGlobalObject*>(&globalObject)->scriptExecutionContext())->settingsValues().testSetting2Enabled) {
...
#if (ENABLE(Condition12) && ENABLE(Condition22)) || ENABLE(Condition23)
    if (!downcast<Document>(jsUncheckedDowncast<JSDOMGlobalObject*>(realm())->scriptExecutionContext())->settingsValues().testSetting2Enabled) {
...
#endif
#if (ENABLE(Condition12) && ENABLE(Condition22)) || ENABLE(Condition23)
    if (!(downcast<Document>(jsUncheckedDowncast<JSDOMGlobalObject*>(realm())->scriptExecutionContext())->settingsValues().testSettingEnabled && downcast<Document>(jsUncheckedDowncast<JSDOMGlobalObject*>(realm())->scriptExecutionContext())->settingsValues().testSetting2Enabled)) {
        hasDisabledRuntimeProperties = true;
...
{
    return getDOMConstructor<JSTestConditionalIncludesDOMConstructor, DOMConstructorID::TestConditionalIncludes>(vm, *jsUncheckedDowncast<const JSDOMGlobalObject*>(globalObject));
}
...
JSC_DEFINE_CUSTOM_GETTER(jsTestConditionalIncludesConstructor, (JSGlobalObject* lexicalGlobalObject, EncodedJSValue thisValue, PropertyName))
{
    ...
    auto* prototype = jsDynamicDowncast<JSTestConditionalIncludesPrototype*>(JSValue::decode(thisValue));
...
{
    auto* thisObject = jsUncheckedDowncast<JSTestConditionalIncludes*>(cell);
...
{
    if (auto* wrapper = jsDynamicDowncast<JSTestConditionalIncludes*>(value))
