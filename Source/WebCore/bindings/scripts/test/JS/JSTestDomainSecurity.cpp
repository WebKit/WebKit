...
JSC::GCClient::IsoSubspace* JSTestDomainSecurity::subspaceForImpl(JSC::VM& vm)
{
    ...
    if (auto* clientSpace = clientSpaces.m_clientSubspaceForTestDomainSecurity.get())
        return clientSpace;
    ...
    IsoSubspace* space = spaces.m_subspaceForTestDomainSecurity.get();
    if (!space) {
        Heap& heap = vm.heap;
        ...
        if constexpr (std::is_base_of_v<JSC::JSDestructibleObject, JSTestDomainSecurity>)
            space = new IsoSubspace ISO_SUBSPACE_INIT(heap, heap.destructibleObjectHeapCellType, JSTestDomainSecurity);
        else
            space = new IsoSubspace ISO_SUBSPACE_INIT(heap, heap.cellHeapCellType, JSTestDomainSecurity);
        spaces.m_subspaceForTestDomainSecurity = std::unique_ptr<IsoSubspace>(space);
...
IGNORE_WARNINGS_END
    ...
    clientSpaces.m_clientSubspaceForTestDomainSecurity = makeUnique<JSC::GCClient::IsoSubspace>(*space);
    return clientSpaces.m_clientSubspaceForTestDomainSecurity.get();
}
