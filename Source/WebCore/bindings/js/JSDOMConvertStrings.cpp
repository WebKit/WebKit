...
ConversionResult<IDLDOMString> trustedTypeCompliantString(TrustedType expectedType, JSGlobalObject& global, JSValue input, const String& sink, ShouldConvertNullToEmptyString shouldConvertNullToEmptyString)
{
    ...
    ConversionResult<IDLDOMString> convertedValue = expectedType == TrustedType::TrustedScriptURL
        ? ConversionResult<IDLDOMString> { Converter<IDLUSVString>::convert(global, input) }
        : ConversionResult<IDLDOMString> { Converter<IDLDOMString>::convert(global, input) };
    RETURN_IF_EXCEPTION(throwScope, ConversionResult<IDLDOMString>::exception());
