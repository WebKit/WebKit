...
JSValue JSDocument::location(ExecState* exec) const
{
    ...
    if (JSDOMWrapper* wrapper = getCachedWrapper(currentWorld(exec), location.get()))
        return wrapper;

    JSLocation* jsLocation = JSLocation::create(getDOMStructure<JSLocation>(exec, globalObject()), globalObject(), location.get());
