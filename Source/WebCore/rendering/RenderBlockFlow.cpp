...
std::optional<LayoutUnit> RenderBlockFlow::firstLineBaseline() const
{
    ...
    // Under size containment the box is sized (and its baseline synthesized) as if it had no
    // contents, so the multicol content baseline must not be exported. See
