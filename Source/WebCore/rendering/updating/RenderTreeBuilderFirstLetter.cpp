...
static RenderStyle styleForFirstLetter(const RenderBlock& firstLetterBlock, const RenderObject& firstLetterContainer)
{
    ...
    if (firstLetterStyle.initialLetterHeight() >= 1 && firstLetterStyle.fontMetricsOfPrimaryFont().hasCapHeight() && paragraph->style().fontMetricsOfPrimaryFont().hasCapHeight()) {
        ...
        float capRatio = firstLetterStyle.fontMetricsOfPrimaryFont().floatCapHeight() / firstLetterStyle.computedFontPixelSize();
        float startingFontSize = ((firstLetterStyle.initialLetterHeight() - 1) * lineHeight + paragraph->style().fontMetricsOfPrimaryFont().capHeight()) / capRatio;
        ...
        int desiredCapHeight = (firstLetterStyle.initialLetterHeight() - 1) * lineHeight + paragraph->style().fontMetricsOfPrimaryFont().capHeight();
        int actualCapHeight = firstLetterStyle.fontMetricsOfPrimaryFont().capHeight();
        while (actualCapHeight > desiredCapHeight) {
            ...
            firstLetterStyle.setFontDescription(WTFMove(newFontDescription));
            firstLetterStyle.fontCascade().update(firstLetterStyle.fontCascade().fontSelector());
            actualCapHeight = firstLetterStyle.fontMetricsOfPrimaryFont().capHeight();
        }
    }
