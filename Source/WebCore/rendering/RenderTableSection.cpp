...
LayoutUnit RenderTableSection::calcBlockDirectionOuterBorder(BlockBorderSide side) const
{
    ...
    if (rowBorder.hasVisibleStyle()) {
        float rowBorderWidth = Style::evaluate<float>(rowBorder.width, style().usedZoomForLength(), deviceScaleFactor);
    ...
    for (unsigned c = 0; c < totalCols; c++) {
        ...
        if (colGroup) {
            ...
            if (colBorder.hasVisibleStyle()) {
                float colBorderWidth = Style::evaluate<float>(colBorder.width, style().usedZoomForLength(), deviceScaleFactor);
                if (colBorderWidth > borderWidth)
                    borderWidth = colBorderWidth;
            }
            if (cellBorder.hasVisibleStyle()) {
                float cellBorderWidth = Style::evaluate<float>(cellBorder.width, style().usedZoomForLength(), deviceScaleFactor);
    ...
    if (allHidden)
        return -1;
    return CollapsedBorderValue::adjustedCollapsedBorderWidth(borderWidth, style().deviceScaleFactor(), (side == BlockBorderSide::BorderAfter));
}
...
LayoutUnit RenderTableSection::calcInlineDirectionOuterBorder(InlineBorderSide side) const
{
    ...
    if (allHidden)
        return -1;
    return CollapsedBorderValue::adjustedCollapsedBorderWidth(borderWidth, style().deviceScaleFactor(), (side == InlineBorderSide::BorderStart) ? writingMode.isInlineFlipped() : !writingMode.isInlineFlipped());
}
    ...
    case BoxSide::Top:
        paintRowGroupBorder(
            ...
            LayoutRect {
                ...
                LayoutUnit { Style::evaluate<float>(style.borderTop().width, style.usedZoomForLength(), deviceScaleFactor) },
            },
            BoxSide::Top,
    ...
    case BoxSide::Bottom:
        paintRowGroupBorder(
            ...
            LayoutRect {
                ...
                horizontalRowGroupBorderWidth(cell, rowGroupRect, row, column),
                LayoutUnit { Style::evaluate<float>(style.borderBottom().width, style.usedZoomForLength(), deviceScaleFactor) },
            },
            BoxSide::Bottom,
    ...
    case BoxSide::Left:
        paintRowGroupBorder(
            ...
            LayoutRect {
                ...
                LayoutUnit { Style::evaluate<float>(style.borderLeft().width, style.usedZoomForLength(), deviceScaleFactor) },
    ...
    case BoxSide::Right:
        paintRowGroupBorder(
            ...
            LayoutRect {
                ...
                rowGroupRect.y() + offsetTopForRowGroupBorder(cell, borderSide, row),
                LayoutUnit { Style::evaluate<float>(style.borderRight().width, style.usedZoomForLength(), deviceScaleFactor) },
                verticalRowGroupBorderHeight(cell, rowGroupRect, row),
            },
            BoxSide::Right,
...
CollapsedBorderValue RenderTableSection::cachedCollapsedBorder(const RenderTableCell& cell, CollapsedBorderSide side)
{
    ...
    if (it == m_cellsCollapsedBorders.end())
        return CollapsedBorderValue(BorderValue(), Color(), BorderPrecedence::Cell, cell.style().usedZoomForLength(), deviceScaleFactor);
    return it->value;
}
