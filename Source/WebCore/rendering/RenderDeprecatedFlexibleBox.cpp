...
void RenderDeprecatedFlexibleBox::computePreferredLogicalWidths()
{
    ...
    if (auto widthFixed = style().width().fixed(); widthFixed && widthFixed->value > 0)
        m_minPreferredLogicalWidth = m_maxPreferredLogicalWidth = adjustContentBoxLogicalWidthForBoxSizing(*widthFixed);
    else
...
RenderDeprecatedFlexibleBox::ClampedContent RenderDeprecatedFlexibleBox::applyLineClamp(FlexBoxIterator& iterator, bool relayoutChildren)
{
    auto initialize = [&] {
        for (RenderBox* child = iterator.first(); child; child = iterator.next()) {
            ...
            child->clearOverridingSize();
            if (relayoutChildren || (child->isReplacedOrAtomicInline() && (child->style().width().isPercentOrCalculated() || child->style().height().isPercentOrCalculated()))
...
void RenderDeprecatedFlexibleBox::clearLineClamp()
{
    ...
    for (RenderBox* child = iterator.first(); child; child = iterator.next()) {
        ...
        child->clearOverridingSize();
        if ((child->isReplacedOrAtomicInline() && (child->style().width().isPercentOrCalculated() || child->style().height().isPercentOrCalculated()))
            || (child->style().height().isAuto() && is<RenderBlockFlow>(*child))) {
...
LayoutUnit RenderDeprecatedFlexibleBox::allowedChildFlex(RenderBox* child, bool expanding, unsigned int group)
{
    ...
    if (expanding) {
        if (isHorizontal()) {
            ...
            if (auto fixedMaxWidth = child->style().maxWidth().fixed())
            ...
            if (auto fixedMaxHeight = child->style().maxHeight().fixed())
    ...
    if (isHorizontal()) {
        ...
        LayoutUnit width = contentWidthForChild(child);
        if (auto fixedMinWidth = child->style().minWidth().fixed())
        ...
        if (auto fixedMinHeight = minHeight.fixed()) {
