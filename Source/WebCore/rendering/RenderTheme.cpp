...
void RenderTheme::paintPlatformResizer(const RenderLayerModelObject& renderer, GraphicsContext& context, const LayoutRect& resizerCornerRect)
{
    ...
    if (renderer.shouldPlaceVerticalScrollbarOnLeft()) {
        ...
        if (resizeCornerImage)
            context.drawBitmapImage(*resizeCornerImage, FloatRect(FloatPoint(), cornerResizerSize), FloatRect { { }, resizeCornerImage->size() });
        return;
    }
    ...
    context.drawBitmapImage(*resizeCornerImage, imageRect, FloatRect { { }, resizeCornerImage->size() });
}
