...
Style::PreferredSize RenderTableCell::logicalWidthFromColumns(RenderTableCol* firstColForThisCell, Style::PreferredSize widthFromStyle) const
{
    ...
    for (unsigned i = 1; i <= colSpanCount; i++) {
        ...
        auto colWidthFixed = colWidth.fixed();
        if (!colWidthFixed) {
            if (colSpanCount > 1)
        ...
        colWidthSum += colWidthFixed->value;
...
void RenderTableCell::computePreferredLogicalWidths()
{
    ...
    auto w = styleOrColLogicalWidth();
    if (auto fixedWidth = w.fixed()) {
        // Nowrap is set, but we didn't actually use it because of the
        // fixed width set on the cell. Even so, it is a WinIE/Moz trait
        // to make the minwidth of the cell into the fixed width. They do this
        // even in strict mode, so do not make this a quirk. Affected the top
        // of hiptop.com.
        m_minPreferredLogicalWidth = std::max(LayoutUnit(fixedWidth->value), m_minPreferredLogicalWidth);
    }
}
