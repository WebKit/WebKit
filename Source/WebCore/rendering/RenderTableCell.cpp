...
CollapsedBorderValue RenderTableCell::emptyBorder() const
{
    ...
    return CollapsedBorderValue(BorderValue(), Color(), BorderPrecedence::Cell, style().usedZoomForLength(), deviceScaleFactor);
}
...
LayoutUnit RenderTableCell::borderHalfStart(bool outer) const
{
    ...
    if (border.exists())
        return CollapsedBorderValue::adjustedCollapsedBorderWidth(border.width(), style().deviceScaleFactor(), !(tableWritingMode().isInlineFlipped() ^ outer));
    return 0;
}
...
LayoutUnit RenderTableCell::borderHalfBefore(bool outer) const
{
    ...
    if (border.exists())
        return CollapsedBorderValue::adjustedCollapsedBorderWidth(border.width(), style().deviceScaleFactor(), !(tableWritingMode().isBlockFlipped() ^ outer));
    return 0;
}
