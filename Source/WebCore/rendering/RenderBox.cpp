...
LayoutUnit RenderBox::adjustBorderBoxLogicalWidthForBoxSizing(LayoutUnit computedLogicalWidth, const Style::Percentage<CSS::Nonnegative, float>&) const
{
...
LayoutUnit RenderBox::adjustBorderBoxLogicalWidthForBoxSizing(LayoutUnit computedLogicalWidth, const Style::UnevaluatedCalculation<CSS::LengthPercentage<CSS::Nonnegative, float>>&) const
{
    return adjustBorderBoxLogicalWidthForBoxSizing(Style::Length<CSS::Nonnegative, float> { computedLogicalWidth });
}

template<typename SizeType> LayoutUnit RenderBox::adjustBorderBoxLogicalWidthForBoxSizing(LayoutUnit computedLogicalWidth, const SizeType& original) const
{
    if (original.isCalculated())
        return adjustBorderBoxLogicalWidthForBoxSizing(Style::Length<CSS::Nonnegative, float> { computedLogicalWidth });
    auto bordersAndPadding = borderAndPaddingLogicalWidth();
    if (style().boxSizing() == BoxSizing::ContentBox || original.isIntrinsicOrLegacyIntrinsicOrAuto())
        return computedLogicalWidth + bordersAndPadding;
    return std::max(computedLogicalWidth, bordersAndPadding);
}
...
LayoutUnit RenderBox::adjustContentBoxLogicalWidthForBoxSizing(LayoutUnit computedLogicalWidth, const Style::Percentage<CSS::Nonnegative, float>&) const
{
    if (style().boxSizing() == BoxSizing::ContentBox)
...
LayoutUnit RenderBox::adjustContentBoxLogicalWidthForBoxSizing(LayoutUnit computedLogicalWidth, const Style::UnevaluatedCalculation<CSS::LengthPercentage<CSS::Nonnegative, float>>&) const
{
    return adjustContentBoxLogicalWidthForBoxSizing(Style::Length<CSS::Nonnegative, float> { computedLogicalWidth });
}

template<typename SizeType> LayoutUnit RenderBox::adjustContentBoxLogicalWidthForBoxSizing(LayoutUnit computedLogicalWidth, const SizeType& original) const
{
    if (original.isCalculated())
        return adjustContentBoxLogicalWidthForBoxSizing(Style::Length<CSS::Nonnegative, float> { computedLogicalWidth });
    if (style().boxSizing() == BoxSizing::ContentBox || original.isIntrinsicOrLegacyIntrinsicOrAuto())
...
template<typename SizeType> LayoutUnit RenderBox::computeLogicalWidthUsingGeneric(const SizeType& logicalWidth, LayoutUnit availableLogicalWidth, const RenderBlock& containingBlock) const
{
    if constexpr (std::same_as<SizeType, Style::MinimumSize>) {
        if (logicalWidth.isAuto())
            return adjustBorderBoxLogicalWidthForBoxSizing(0, logicalWidth);
    }

    if (!logicalWidth.isIntrinsicOrLegacyIntrinsicOrAuto()) {
        // FIXME: If the containing block flow is perpendicular to our direction we need to use the available logical height instead.
        return adjustBorderBoxLogicalWidthForBoxSizing(Style::evaluate(logicalWidth, availableLogicalWidth), logicalWidth);
    }
...
std::optional<LayoutUnit> RenderBox::computeIntrinsicLogicalContentHeightUsing(CSS::Keyword::WebkitFillAvailable, std::optional<LayoutUnit>, LayoutUnit borderAndPadding) const
{
    return containingBlock()->availableLogicalHeight(AvailableLogicalHeightType::ExcludeMarginBorderPadding) - borderAndPadding;
}

std::optional<LayoutUnit> RenderBox::computeIntrinsicLogicalContentHeightUsing(CSS::Keyword::Intrinsic, std::optional<LayoutUnit> intrinsicContentHeight, LayoutUnit) const
{
...
std::optional<LayoutUnit> RenderBox::computeIntrinsicLogicalContentHeightUsing(CSS::Keyword::MinIntrinsic, std::optional<LayoutUnit> intrinsicContentHeight, LayoutUnit) const
{
    if (intrinsicContentHeight)
        return adjustIntrinsicLogicalHeightForBoxSizing(*intrinsicContentHeight);
    return { };
...
std::optional<LayoutUnit> RenderBox::computeIntrinsicLogicalContentHeightUsing(CSS::Keyword::FitContent, std::optional<LayoutUnit> intrinsicContentHeight, LayoutUnit) const
{
    if (intrinsicContentHeight)
        return adjustIntrinsicLogicalHeightForBoxSizing(*intrinsicContentHeight);
    return { };
...
std::optional<LayoutUnit> RenderBox::computeIntrinsicLogicalContentHeightUsing(CSS::Keyword::MaxContent, std::optional<LayoutUnit> intrinsicContentHeight, LayoutUnit) const
{
...
std::optional<LayoutUnit> RenderBox::computeIntrinsicLogicalContentHeightUsing(CSS::Keyword::MinContent, std::optional<LayoutUnit> intrinsicContentHeight, LayoutUnit) const
{
    // FIXME: The CSS sizing spec is considering changing what min-content/max-content should resolve to.
    // If that happens, this code will have to change.
    if (auto* renderImage = dynamicDowncast<RenderImage>(this)) {
...
template<typename SizeType> std::optional<LayoutUnit> RenderBox::computeIntrinsicLogicalContentHeightUsingGeneric(const SizeType& logicalHeight, std::optional<LayoutUnit> intrinsicContentHeight, LayoutUnit borderAndPadding) const
{
    return WTF::switchOn(logicalHeight,
        [&](const CSS::Keyword::MinContent& keyword) -> std::optional<LayoutUnit> {
            return computeIntrinsicLogicalContentHeightUsing(keyword, intrinsicContentHeight, borderAndPadding);
        },
        [&](const CSS::Keyword::MaxContent& keyword) -> std::optional<LayoutUnit> {
            return computeIntrinsicLogicalContentHeightUsing(keyword, intrinsicContentHeight, borderAndPadding);
        },
        [&](const CSS::Keyword::Intrinsic& keyword) -> std::optional<LayoutUnit> {
            return computeIntrinsicLogicalContentHeightUsing(keyword, intrinsicContentHeight, borderAndPadding);
        },
        [&](const CSS::Keyword::MinIntrinsic& keyword) -> std::optional<LayoutUnit> {
            return computeIntrinsicLogicalContentHeightUsing(keyword, intrinsicContentHeight, borderAndPadding);
        },
        [&](const CSS::Keyword::FitContent& keyword) -> std::optional<LayoutUnit> {
            return computeIntrinsicLogicalContentHeightUsing(keyword, intrinsicContentHeight, borderAndPadding);
        },
        [&](const CSS::Keyword::WebkitFillAvailable& keyword) -> std::optional<LayoutUnit> {
            return computeIntrinsicLogicalContentHeightUsing(keyword, intrinsicContentHeight, borderAndPadding);
        },
...
template<typename SizeType> std::optional<LayoutUnit> RenderBox::computeContentAndScrollbarLogicalHeightUsing(const SizeType& logicalHeight, std::optional<LayoutUnit> intrinsicContentHeight) const
{
    return WTF::switchOn(logicalHeight,
        ...
        [&](const typename SizeType::Percentage& percentageLogicalHeight) -> std::optional<LayoutUnit> {
            return computePercentageLogicalHeight(percentageLogicalHeight);
        },
        [&](const typename SizeType::Calc& calcLogicalHeight) -> std::optional<LayoutUnit> {
            return computePercentageLogicalHeight(calcLogicalHeight);
        },
...
template<typename SizeType> LayoutUnit RenderBox::computeReplacedLogicalWidthUsingGeneric(const SizeType& logicalWidth) const
{
    if constexpr (std::same_as<SizeType, Style::MinimumSize>) {
        if (logicalWidth.isAuto())
            return adjustContentBoxLogicalWidthForBoxSizing(0, logicalWidth);
    }
    ...
    auto calculated = [&] {
        ...
        if (containerWidth > 0 || (!containerWidth && (containerLogicalWidth.isSpecified())))
            return adjustContentBoxLogicalWidthForBoxSizing(Style::evaluateMinimum(logicalWidth, containerWidth), logicalWidth);
        return 0_lu;
    };
...
{
    auto logicalWidth = overridingLogicalWidthForFlexBasisComputation().value_or(style().logicalWidth());
...
template<typename SizeType> LayoutUnit RenderBox::computeReplacedLogicalHeightUsingGeneric(const SizeType& logicalHeight) const
{
    ...
    auto percentageOrCalculated = [&](const auto& percentageOrCalcLogicalHeight) {
        ...
        if (container->isOutOfFlowPositioned()
            ...
            return adjustContentBoxLogicalHeightForBoxSizing(Style::evaluate(percentageOrCalcLogicalHeight, newContentHeight));
        }
        ...
        if (isOutOfFlowPositioned()) {
            ...
            while (container && !is<RenderView>(*container)
                ...
                if (container->isRenderTableCell()) {
                    ...
                    return Style::evaluate(percentageOrCalcLogicalHeight, availableHeight - borderAndPaddingLogicalHeight());
                }
                downcast<RenderBlock>(*container).addPercentHeightDescendant(const_cast<RenderBox&>(*this));
                container = container->containingBlock();
            }
        }
        return adjustContentBoxLogicalHeightForBoxSizing(Style::evaluate(percentageOrCalcLogicalHeight, availableHeight));
    };
    ...
    return WTF::switchOn(logicalHeight,
        ...
        [&](const typename SizeType::Calc& calcLogicalHeight) -> LayoutUnit {
            return percentageOrCalculated(calcLogicalHeight);
        },
        [&](const CSS::Keyword::FitContent& keyword) -> LayoutUnit {
            return adjustContentBoxLogicalHeightForBoxSizing(computeIntrinsicLogicalContentHeightUsing(keyword, intrinsicLogicalHeight(), borderAndPaddingLogicalHeight()));
        },
        ...
        [&](const CSS::Keyword::Intrinsic& keyword) -> LayoutUnit {
            return adjustContentBoxLogicalHeightForBoxSizing(computeIntrinsicLogicalContentHeightUsing(keyword, intrinsicLogicalHeight(), borderAndPaddingLogicalHeight()));
        },
        [&](const CSS::Keyword::MinIntrinsic& keyword) -> LayoutUnit {
            return adjustContentBoxLogicalHeightForBoxSizing(computeIntrinsicLogicalContentHeightUsing(keyword, intrinsicLogicalHeight(), borderAndPaddingLogicalHeight()));
        },
        [&](const CSS::Keyword::Auto&) -> LayoutUnit  {
            if constexpr (std::same_as<SizeType, Style::MinimumSize>)
...
void RenderBox::computePositionedLogicalWidth(LogicalExtentComputedValues& computedValues) const
{
    ...
    if (!styleToUse.logicalMaxWidth().isNone())
        usedMaxWidth = computePositionedLogicalWidthUsing(styleToUse.logicalMaxWidth(), inlineConstraints);
    ...
    if (!styleToUse.logicalMinWidth().isZero() || styleToUse.logicalMinWidth().isIntrinsic())
        usedMinWidth = computePositionedLogicalWidthUsing(styleToUse.logicalMinWidth(), inlineConstraints);
...
template<typename SizeType> LayoutUnit RenderBox::computePositionedLogicalWidthUsing(const SizeType& originalLogicalWidth, const PositionedLayoutConstraints& inlineConstraints) const
{
    ...
    if (logicalWidth.isIntrinsic()) {
        ...
        logicalWidth = typename SizeType::Fixed { computeIntrinsicLogicalWidthUsing(logicalWidth, availableSpace, inlineConstraints.bordersPlusPadding()) - inlineConstraints.bordersPlusPadding() };
    }
    ...
    if (!logicalWidthIsAuto)
        return adjustContentBoxLogicalWidthForBoxSizing(Style::evaluate(logicalWidth, inlineConstraints.containingSize()), originalLogicalWidth);
...
void RenderBox::computePositionedLogicalHeight(LogicalExtentComputedValues& computedValues) const
{
    ...
    // Clamp by max height.
    if (!styleToUse.logicalMaxHeight().isNone()) {
        auto usedMaxHeight = computePositionedLogicalHeightUsing(styleToUse.logicalMaxHeight(), computedHeight, blockConstraints);
    ...
    if (logicalMinHeight.isAuto() || !logicalMinHeight.isZero() || logicalMinHeight.isIntrinsic()) {
        auto usedMinHeight = computePositionedLogicalHeightUsing(styleToUse.logicalMinHeight(), computedHeight, blockConstraints);
...
template<typename SizeType> LayoutUnit RenderBox::computePositionedLogicalHeightUsing(const SizeType& originalLogicalHeight, LayoutUnit computedHeight, const PositionedLayoutConstraints& blockConstraints) const
{
    ...
    if constexpr (std::same_as<SizeType, Style::MinimumSize>) {
        if (logicalHeight.isAuto()) {
            if (shouldComputeLogicalHeightFromAspectRatio())
                logicalHeight = typename SizeType::Fixed { computedHeight };
            else
                logicalHeight = 0_css_px;
        }
    }

    bool fromAspectRatio = [&] {
        if constexpr (std::same_as<SizeType, Style::PreferredSize>)
            return shouldComputeLogicalHeightFromAspectRatio();
        else
            return false;
    }();
    bool logicalHeightIsAuto = [&] {
        if constexpr (std::same_as<SizeType, Style::MaximumSize>)
            return false;
        else
            return logicalHeight.isAuto() && !fromAspectRatio;
    }();
