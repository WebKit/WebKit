...
void RenderTable::layoutCaption(RenderTableCaption& caption)
{
    ...
    if (caption.needsLayout()) {
        ...
        caption.setLogicalLocation(LayoutPoint(caption.marginStart(caption.writingMode()), caption.marginBefore(caption.writingMode()) + logicalHeight()));
    ...
    setLogicalHeight(logicalHeight() + captionLogicalHeightInTableWritingMode + caption.marginBefore(caption.writingMode()) + caption.marginAfter(caption.writingMode()));
}
...
LayoutUnit RenderTable::sumCaptionsLogicalHeight() const
{
    LayoutUnit height;
    for (auto& caption : m_captions)
        height += caption->logicalHeight() + caption->marginBefore(caption->writingMode()) + caption->marginAfter(caption->writingMode());
    return height;
}
    ...
    LayoutRepainter repainter(*this);
    {
        ...
        for (auto& caption : m_captions) {
            if (caption->style().captionSide() == CaptionSide::Bottom)
                continue;
            oldTableLogicalTop += caption->logicalHeight() + caption->marginBefore(caption->writingMode()) + caption->marginAfter(caption->writingMode());
        }
...
{
    for (auto& caption : m_captions) {
        LayoutUnit captionLogicalHeight = caption->logicalHeight() + caption->marginBefore(caption->writingMode()) + caption->marginAfter(caption->writingMode());
