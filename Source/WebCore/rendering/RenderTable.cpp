...
void RenderTable::computePreferredLogicalWidths()
{
    ...
    for (unsigned i = 0; i < m_captions.size(); i++) {
        ...
        auto captionStyleZoom = captionStyle.usedZoomForLength();
        if (auto fixedMarginStart = captionStyle.marginStart().tryFixed())
            captionMinWidth += Style::evaluate<LayoutUnit>(*fixedMarginStart, captionStyleZoom);
        if (auto fixedMarginEnd = captionStyle.marginEnd().tryFixed())
            captionMinWidth += Style::evaluate<LayoutUnit>(*fixedMarginEnd, captionStyleZoom);
