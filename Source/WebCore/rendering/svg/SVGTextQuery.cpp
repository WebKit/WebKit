...
static inline float lengthAdjustInlineScale(const SVGTextQuery::Data& queryData, const SVGTextFragment& fragment)
{
    if (fragment.lengthAdjustTransform.isIdentity())
        return 1;
    return queryData.isVerticalText ? fragment.lengthAdjustTransform.yScale() : fragment.lengthAdjustTransform.xScale();
}
