...
bool RenderEmbeddedObject::getReplacementTextGeometry(int tx, int ty, FloatRect& contentRect, Path& path, FloatRect& replacementTextRect, Font& font, TextRun& run, float& textWidth)
{
    ...
    run = TextRun(m_unavailablePluginReplacementText.characters(), m_unavailablePluginReplacementText.length());
...
bool RenderEmbeddedObject::isInUnavailablePluginIndicator(MouseEvent* event)
{
    FloatRect contentRect;
    Path path;
    FloatRect replacementTextRect;
