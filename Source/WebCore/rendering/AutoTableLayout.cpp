...
void AutoTableLayout::layout()
{
    ...
    if (available < 0)
        available = shrinkCellWidth<CSS::Keyword::Auto>(available);
    if (available < 0)
        available = shrinkCellWidth<Style::PreferredSize::Fixed>(available);
    if (available < 0)
        available = shrinkCellWidth<Style::PreferredSize::Percentage>(available);
...
template<typename SizeType> float AutoTableLayout::shrinkCellWidth(float available)
{
    auto nEffCols = m_table->numEffCols();
    auto logicalWidthBeyondMin = 0.0f;
    for (auto i = nEffCols; i; ) {
        --i;
        if (m_layoutStruct[i].effectiveLogicalWidth.holdsAlternative<SizeType>())
            logicalWidthBeyondMin += m_layoutStruct[i].computedLogicalWidth - m_layoutStruct[i].effectiveMinLogicalWidth;
    }

    for (auto i = nEffCols; i && logicalWidthBeyondMin > 0; ) {
        --i;
        if (m_layoutStruct[i].effectiveLogicalWidth.holdsAlternative<SizeType>()) {
            auto minMaxDiff = m_layoutStruct[i].computedLogicalWidth - m_layoutStruct[i].effectiveMinLogicalWidth;
            auto reduce = available * minMaxDiff / logicalWidthBeyondMin;
