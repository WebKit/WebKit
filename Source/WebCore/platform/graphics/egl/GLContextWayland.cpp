...
GLContext::GLContext(PlatformDisplay& display, EGLContext context, EGLSurface surface, EGLConfig config, WlUniquePtr<struct wl_surface>&& wlSurface, struct wl_egl_window* wlWindow)
    : m_display(display)
    , m_context(context)
    , m_surface(surface)
    , m_config(config)
...
EGLSurface GLContext::createWindowSurfaceWayland(EGLDisplay display, EGLConfig config, GLNativeWindowType window)
{
...
std::unique_ptr<GLContext> GLContext::createWaylandContext(PlatformDisplay& platformDisplay, EGLContext sharingContext)
{
    ...
    return makeUnique<GLContext>(platformDisplay, context, surface, config, WTFMove(wlSurface), window);
}

void GLContext::destroyWaylandWindow()
{
    if (m_wlWindow)
