...
static void copyToGstBufferPlane(std::span<uint8_t> destination, const GstVideoInfo& info, size_t planeIndex, std::span<const uint8_t> source, size_t rowCount, uint32_t bytesPerRowSource, uint32_t bytesPerRowToCopy)
{
    ...
    // Copy each visible row, stepping the source by its coded row stride so a cropped frame reads
    // each row from the correct position.
...
// Source base = coded plane offset (destinationOffset) shifted by the visibleRect crop origin.
...
RefPtr<VideoFrame> VideoFrame::createI420(std::span<const uint8_t> span, size_t width, size_t height, const ComputedPlaneLayout& planeY, const ComputedPlaneLayout& planeU, const ComputedPlaneLayout& planeV, PlatformVideoColorSpace&& colorSpace)
{
    ...
    gst_buffer_memset(buffer.get(), 0, 0, span.size_bytes());
    {
        ...
        // Read each plane from its crop-adjusted source base (see planeSourceOffset).
