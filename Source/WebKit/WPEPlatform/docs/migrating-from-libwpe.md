...
## 6. Extensions without a counterpart yet

Two WPEBackend-fdo extensions that some applications relied on have no
WPEPlatform counterpart yet: `<wpe/extensions/audio.h>` (external audio
rendering) and `<wpe/extensions/video-plane-display-dmabuf.h>` (hardware
video overlay planes, used on Broadcom-class SoCs). Applications that
depend on them should keep that functionality on the old stack for now,
and raise the gap with the project before porting.
