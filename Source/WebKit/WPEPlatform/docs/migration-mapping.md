...
| `wpe_view_activity_state_focused`, which WebKit also used to derive `ActivityState::WindowIsActive` | `WPE_TOPLEVEL_STATE_ACTIVE` from [method@Toplevel.get_state], observed per-view via [signal@View::toplevel-state-changed] | Reshape | libwpe had no active-window state, so WebKit treated a focused view as an active window. WPEPlatform reports window activation separately from view focus, as a [flags@ToplevelState] bit. |
...
| `wpe_gamepad_provider_start()` / `_stop()` | — | Removed | Lifecycle is implicit: the manager is active from creation until its last reference is dropped. [method@Display.create_gamepad_manager] returns a new reference owned by the caller, and falls back to a libmanette-based manager only when WebKit is built with libmanette. |
...
| `wpe_process_launch()` / `wpe_process_terminate()` | — | Removed | Child-process launch is internal to WPE WebKit again, and platform implementations no longer take part. The exception is Android, where WPEPlatform provides `WPEProcessManager` (`wpe_process_manager_launch()` / `_terminate()`). It is only built and installed for Android. |
...
| Version macros (`WPE_MAJOR_VERSION`, `WPE_CHECK_VERSION`, etc.) | `WPE_PLATFORM_MAJOR_VERSION`, `WPE_PLATFORM_MINOR_VERSION`, `WPE_PLATFORM_MICRO_VERSION`, `WPE_PLATFORM_CHECK_VERSION` (generated `WPEVersion.h`, pulled in by `<wpe/wpe-platform.h>`) | Reshape | Renamed with a `PLATFORM` infix. Runtime versions are available from `wpe_platform_get_major_version()` and friends. |
...
| `wpe_view_backend_exportable_fdo_get_view_backend()` | — | Removed | No wrapper object: the [class@View] subclass itself is what WebKit renders into. |
