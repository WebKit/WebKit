...
static void webkitWebsiteDataManagerSetProperty(GObject* object, guint propID, const GValue* value, GParamSpec* paramSpec)
{
    ...
    case PROP_IS_EPHEMERAL:
        if (g_value_get_boolean(value))
            manager->priv->websiteDataStore = WebsiteDataStore::createNonPersistent();
        break;
    default:
        G_OBJECT_WARN_INVALID_PROPERTY_ID(object, propID, paramSpec);
    }
...
WebsiteDataStore& webkitWebsiteDataManagerGetDataStore(WebKitWebsiteDataManager* manager)
{
    ...
    if (!priv->websiteDataStore) {
        ...
        configuration->setLocalStorageDirectory(!priv->localStorageDirectory ?
            WebsiteDataStore::defaultLocalStorageDirectory() : FileSystem::stringFromFileSystemRepresentation(priv->localStorageDirectory.get()));
        configuration->setNetworkCacheDirectory(!priv->diskCacheDirectory ?
            WebsiteDataStore::defaultNetworkCacheDirectory() : FileSystem::pathByAppendingComponent(FileSystem::stringFromFileSystemRepresentation(priv->diskCacheDirectory.get()), networkCacheSubdirectory));
        configuration->setApplicationCacheDirectory(!priv->applicationCacheDirectory ?
            WebsiteDataStore::defaultApplicationCacheDirectory() : FileSystem::stringFromFileSystemRepresentation(priv->applicationCacheDirectory.get()));
        configuration->setIndexedDBDatabaseDirectory(!priv->indexedDBDirectory ?
            WebsiteDataStore::defaultIndexedDBDatabaseDirectory() : FileSystem::stringFromFileSystemRepresentation(priv->indexedDBDirectory.get()));
        configuration->setWebSQLDatabaseDirectory(!priv->webSQLDirectory ?
            WebsiteDataStore::defaultWebSQLDatabaseDirectory() : FileSystem::stringFromFileSystemRepresentation(priv->webSQLDirectory.get()));
        configuration->setHSTSStorageDirectory(!priv->hstsCacheDirectory ?
            WebsiteDataStore::defaultHSTSDirectory() : FileSystem::stringFromFileSystemRepresentation(priv->hstsCacheDirectory.get()));
        configuration->setMediaKeysStorageDirectory(WebsiteDataStore::defaultMediaKeysStorageDirectory());
        priv->websiteDataStore = WebsiteDataStore::create(WTFMove(configuration), PAL::SessionID::defaultSessionID());
    }
...
const gchar* webkit_website_data_manager_get_local_storage_directory(WebKitWebsiteDataManager* manager)
{
    ...
    if (!priv->localStorageDirectory)
        priv->localStorageDirectory.reset(g_strdup(WebsiteDataStore::defaultLocalStorageDirectory().utf8().data()));
...
const gchar* webkit_website_data_manager_get_disk_cache_directory(WebKitWebsiteDataManager* manager)
{
    ...
    if (!priv->diskCacheDirectory) {
        ...
        priv->diskCacheDirectory.reset(g_strdup(FileSystem::directoryName(WebsiteDataStore::defaultNetworkCacheDirectory()).utf8().data()));
    }
...
const gchar* webkit_website_data_manager_get_offline_application_cache_directory(WebKitWebsiteDataManager* manager)
{
    ...
    if (!priv->applicationCacheDirectory)
        priv->applicationCacheDirectory.reset(g_strdup(WebsiteDataStore::defaultApplicationCacheDirectory().utf8().data()));
...
const gchar* webkit_website_data_manager_get_indexeddb_directory(WebKitWebsiteDataManager* manager)
{
    ...
    if (!priv->indexedDBDirectory)
        priv->indexedDBDirectory.reset(g_strdup(WebsiteDataStore::defaultIndexedDBDatabaseDirectory().utf8().data()));
...
const gchar* webkit_website_data_manager_get_websql_directory(WebKitWebsiteDataManager* manager)
{
    ...
    if (!priv->webSQLDirectory)
        priv->webSQLDirectory.reset(g_strdup(WebsiteDataStore::defaultWebSQLDatabaseDirectory().utf8().data()));
...
const gchar* webkit_website_data_manager_get_hsts_cache_directory(WebKitWebsiteDataManager* manager)
{
    ...
    if (!priv->hstsCacheDirectory)
        priv->hstsCacheDirectory.reset(g_strdup(WebsiteDataStore::defaultHSTSDirectory().utf8().data()));
