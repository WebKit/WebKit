...
static void webKitFindControllerPerform(WebKitFindController* findController, WebKitFindControllerOperation operation)
{
    ...
    if (operation == CountOperation) {
        getPage(findController).countStringMatches(priv->searchText, priv->findOptions, priv->maxMatchCount);
        return;
    }
    ...
    getPage(findController).findString(priv->searchText, findOptions, priv->maxMatchCount);
}
...
{
    findController->priv->searchText = UTF8CString(searchText);
