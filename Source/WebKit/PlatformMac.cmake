...
function(WEBKIT_DEFINE_XPC_SERVICES)
    ...
    function(WEBKIT_WEBCONTENT_VARIANT _variant)
        ...
        set(_bundle_id "com.apple.WebKit.WebContent.${_variant}")
        set(_exec_name "${_bundle_id}.Development")
        if (ARGC GREATER_EQUAL 2 AND NOT "${ARGV1}" STREQUAL "")
    ...
    WEBKIT_WEBCONTENT_VARIANT(Development "com.apple.WebKit.WebContent.Development")

    # Sign each WebContent XPC bundle with entitlements generated by the same
    # Scripts/process-entitlements.sh that Xcode runs, so we pick up future changes.
    set(_entitlements_script ${WEBKIT_DIR}/Scripts/process-entitlements.sh)
    set(_entitlements_deps
        ${_entitlements_script}
    ...
    set(_entitlements_dir ${CMAKE_BINARY_DIR}/Entitlements)
    file(MAKE_DIRECTORY ${_entitlements_dir})
    # process-entitlements.sh expects TARGET_MAC_OS_X_VERSION_MAJOR / SDK_VERSION_ACTUAL
    # in Xcode's encoding (MAJOR * 10000).
    if (NOT DEFINED WEBKIT_MAC_SDK_VERSION)
        execute_process(
            COMMAND xcrun --sdk macosx --show-sdk-version
            OUTPUT_VARIABLE WEBKIT_MAC_SDK_VERSION
            OUTPUT_STRIP_TRAILING_WHITESPACE)
        set(WEBKIT_MAC_SDK_VERSION "${WEBKIT_MAC_SDK_VERSION}" CACHE INTERNAL "macOS SDK version (e.g. 27.0)")
        string(REGEX MATCH "^([0-9]+)" _sdk_major "${WEBKIT_MAC_SDK_VERSION}")
        math(EXPR _target_mac_os_x_version_major "${CMAKE_MATCH_1} * 10000")
        set(WEBKIT_TARGET_MAC_OS_X_VERSION_MAJOR "${_target_mac_os_x_version_major}" CACHE INTERNAL "TARGET_MAC_OS_X_VERSION_MAJOR")
        set(WEBKIT_SDK_VERSION_ACTUAL "${_target_mac_os_x_version_major}" CACHE INTERNAL "SDK_VERSION_ACTUAL")
    endif ()

    function(WEBKIT_SIGN_WEBCONTENT_XPC _target _bundle_id _variant)
        # PRODUCT_NAME is process-entitlements.sh's dispatch key; must match the bundle id.
        set(_xcent ${_entitlements_dir}/${_bundle_id}.xcent)
        set(_service_dir ${WebKit_XPC_SERVICE_DIR}/${_bundle_id}.xpc)
        set(_codesign_stamp ${_entitlements_dir}/${_bundle_id}.codesigned.stamp)

        add_custom_command(
            OUTPUT ${_xcent}
            DEPENDS ${_entitlements_deps}
            COMMAND ${CMAKE_COMMAND} -E env
                WK_PROCESSED_XCENT_FILE=${_xcent}
                WK_USE_RESTRICTED_ENTITLEMENTS=NO
                WK_USE_FATAL_EXCEPTIONS=YES
                WK_XPC_SERVICE_VARIANT=${_variant}
                WK_PLATFORM_NAME=macosx
                PRODUCT_NAME=${_bundle_id}
                PRODUCT_BUNDLE_IDENTIFIER=${_bundle_id}
                SDK_VERSION_ACTUAL=${WEBKIT_SDK_VERSION_ACTUAL}
                TARGET_MAC_OS_X_VERSION_MAJOR=${WEBKIT_TARGET_MAC_OS_X_VERSION_MAJOR}
                RC_XBS=
                SDKROOT=${CMAKE_OSX_SYSROOT}
                BUILT_PRODUCTS_DIR=${CMAKE_BINARY_DIR}
                bash ${_entitlements_script}
            WORKING_DIRECTORY ${WEBKIT_DIR}
            COMMENT "Generating entitlements for ${_bundle_id}"
            VERBATIM)

        add_custom_command(
            OUTPUT ${_codesign_stamp}
            DEPENDS ${_xcent} $<TARGET_FILE:${_target}>
            COMMAND codesign --force --sign - --timestamp=none
                --entitlements ${_xcent}
                ${_service_dir}
            COMMAND ${CMAKE_COMMAND} -E touch ${_codesign_stamp}
            COMMENT "Codesigning ${_bundle_id}.xpc"
            VERBATIM)
        add_custom_target(${_target}_Codesign ALL DEPENDS ${_codesign_stamp})
    endfunction()

    WEBKIT_SIGN_WEBCONTENT_XPC(WebProcess
        "com.apple.WebKit.WebContent" Normal)
    WEBKIT_SIGN_WEBCONTENT_XPC(WebProcessEnhancedSecurity
        "com.apple.WebKit.WebContent.EnhancedSecurity" EnhancedSecurity)
    WEBKIT_SIGN_WEBCONTENT_XPC(WebProcessCaptivePortal
        "com.apple.WebKit.WebContent.CaptivePortal" CaptivePortal)
    WEBKIT_SIGN_WEBCONTENT_XPC(WebProcessDevelopment
        "com.apple.WebKit.WebContent.Development" Development)
