...
// Tests for wrapKey and unwrapKey round tripping
    ...
    return Promise.allSettled([generateWrappingKeys(), generateKeysToWrap()])
    .then(function(results) {
        wrappingKeysParameters.filter((param) => Object.keys(wrappers).includes(param.name)).forEach(function(wrapperParam) {
            ...
            var keysToWrap = keysToWrapParameters.assymetricKeys.concat(keysToWrapParameters.symmetricKeys);
            keysToWrap.filter((param) => Object.keys(keys).includes(param.algorithm.name)).forEach(function(toWrapParam) {
                var keyData = keys[toWrapParam.algorithm.name];
                ["raw", "spki", "pkcs8"].filter((fmt) => Object.keys(keyData).includes(fmt)).forEach(function(format) {
                    var toWrap = keyData[format];
                    [format, "jwk"].forEach(function(test) {
                        if (wrappingIsPossible(toWrap.originalExport[test], wrapper.parameters.name)) {
                            testWrapping(wrapper, toWrap, test);
                            if (canCompareNonExtractableKeys(toWrap.key)) {
                                testWrappingNonExtractable(wrapper, toWrap, test);
                                if (test === "jwk") {
                                    testWrappingNonExtractableAsExtractable(wrapper, toWrap, format);
                                }
                            }
                        }
                    });
    ...
    function importAndExport(format, keyData, algorithm, keyUsages, keyType) {
        var importedKey;
        return subtle.importKey(format, keyData, algorithm, true, keyUsages)
            .then((result) => {
                importedKey = result;
                ...
                return subtle.exportKey(format, importedKey);
            })
            .then((exportedKey) => {
            ...
            .then((exportedKey) => {
                keys[algorithm.name][format].originalExport["jwk"] = exportedKey;
            });
    }

    function generateKeysToWrap() {
        var promises = [];
        keysToWrapParameters.assymetricKeys.forEach(function(params) {
        ...
        keysToWrapParameters.symmetricKeys.forEach(function(params) {
            keys[params.algorithm.name] = {};
    ...
    function testWrappingNonExtractableAsExtractable(wrapper, toWrap, fmt) {
        promise_test(async() => {
            var wrappedKey;
            try {
